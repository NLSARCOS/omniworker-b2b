...
export const metadata: Metadata = {
  title: "Asistente Digital de Cobranzas — Flux Agent",
  description: "Cobrá más rápido sin molestar. Recordatorios automáticos, acuerdos de pago y seguimiento inteligente de cartera. By Simplex Latam.",
  keywords: "automatización cobranzas, recordatorios pago automáticos, gestión cartera cobro, cobranzas WhatsApp LATAM",
  openGraph: {
    title: "Asistente Digital de Cobranzas — Flux Agent",
    description: "Cobrá más rápido sin molestar. Recordatorios automáticos, acuerdos de pago y seguimiento inteligente de cartera. By Simplex Latam.",
    url: "https://flux.simplex.lat/asistente-digital-cobranzas",
    siteName: "Flux Agent",
    locale: "es_LA",
    type: "website",
  },
...
export default function Page() {
  const STATS = [{"num": "40%", "label": "Reducción en pagos atrasados"}, {"num": "3x", "label": "Más rápido que el proceso manual"}, {"num": "24/7", "label": "Seguimiento constante"}, {"num": "92%", "label": "Tasa de respuesta positiva"}];
  const FEATURES = [{"icon": "💰", "name": "Recordatorios antes del vencimiento", "dept": "Preventivo"}, {"icon": "📱", "name": "Cobro por WhatsApp y email", "dept": "Multi-canal"}, {"icon": "📋", "name": "Acuerdos de pago automáticos", "dept": "Negociación"}, {"icon": "📊", "name": "Dashboard de cartera en tiempo real", "dept": "Datos"}, {"icon": "🔔", "name": "Alertas de cuentas críticas", "dept": "Riesgo"}, {"icon": "📈", "name": "Seguimiento de promesas de pago", "dept": "Control"}, {"icon": "🔗", "name": "Integración con tu ERP", "dept": "Sistemas"}, {"icon": "✏️", "name": "¿Otro flujo? Lo configuramos", "dept": "Personalizado", "dark": true}];
  const STEPS = [{"n": "01", "title": "Conectamos tu sistema", "desc": "Integramos el asistente con tu ERP, facturación o planilla de cartera. Detecta vencimientos automáticamente."}, {"n": "02", "title": "Configuramos la estrategia", "desc": "Definimos tono, frecuencia y canales de los recordatorios. Progresivo y sin molestar."}, {"n": "03", "title": "Cobra y reporta", "desc": "El asistente envía recordatorios, gestiona acuerdos y escala lo complejo al equipo."}];
  const TESTIMONIALS = [{"quote": "Recuperamos USD 45,000 en el primer trimestre que teníamos perdidos. El asistente es más persistente que cualquier persona, sin generar conflicto.", "name": "Mauricio Salinas", "role": "CFO · CreditosYA Colombia", "result": "↑ USD 45,000 recuperados"}, {"quote": "La mora bajó del 18% al 7% en 4 meses. Los clientes prefieren recibir un mensaje oportuno a una llamada incómoda.", "name": "Claudia Vega", "role": "Gerente Finanzas · PagosEC Ecuador", "result": "↓ Morosidad del 18% al 7%"}, {"quote": "Nuestro equipo de cobranzas pasó de perseguir pagos a analizar riesgo. El asistente hace el trabajo repetitivo.", "name": "Ricardo Ramos", "role": "Director Financiero · FinVault México", "result": "↑ Equipo enfocado en estrategia"}];
  const MARQUEE = ["Recordatorios progresivos", "Acuerdos de pago", "Seguimiento automático", "WhatsApp y email", "Escalado inteligente", "Reportes de cartera", "Gestión de promesas", "Alertas de vencimiento"];
  const PROBLEMA_STATS = [{"num": "30%", "desc": "De la cartera vencida en LATAM nunca se cobra por falta de seguimiento oportuno. El asistente lo evita."}, {"num": "5\nintentos", "desc": "Necesita un deudor promedio antes de pagar. Sin automatización, la mayoría recibe solo 1 recordatorio."}, {"num": "USD\\n25K", "desc": "Recupera una empresa promedio al mes al automatizar cobranzas con un asistente digital inteligente."}];
    ...
    <>
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "name": "Asistente Digital de Cobranzas — Flux Agent",
          "description": "Cobrá más rápido sin molestar. Recordatorios automáticos, acuerdos de pago y seguimiento inteligente de cartera. By Simplex Latam.",
      ...
      <div style={{ background: "var(--paper)", color: "var(--ink)", fontFamily: "'Inter', sans-serif", fontSize: 16, lineHeight: 1.65, WebkitFontSmoothing: "antialiased", backgroundImage: "repeating-linear-gradient(0deg, transparent 0, transparent 23px, rgba(0,0,0,0.022) 23px, rgba(0,0,0,0.022) 24px)" }}>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "72px 5vw 88px", display: "grid", gridTemplateColumns: "1fr 360px", gap: 80, alignItems: "start" }}>
          <div>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--neon-dim)", background: "var(--neon-pale)", padding: "8px 14px", display: "inline-block", marginBottom: 28, borderLeft: `3px solid var(--neon)` }}>
              Cobranzas inteligentes · By Simplex Latam
            </div>
            <h1 style={{ fontFamily: "'Fraunces', serif", fontVariationSettings: '"opsz" 144, "SOFT" 30, "WONK" 0', fontWeight: 500, fontSize: "clamp(52px, 6vw, 88px)", lineHeight: 0.93, letterSpacing: "-0.04em", color: "var(--ink)", marginBottom: 32 }}>Cobrá más rápido,<br />{" "}<em style={{ fontStyle: "italic", color: "var(--neon-dim)", fontVariationSettings: '"opsz" 144, "SOFT" 80' }}>sin molestar.</em></h1>
            <p style={{ fontSize: 20, fontWeight: 400, lineHeight: 1.6, color: "var(--ink-soft)", maxWidth: 600, marginBottom: 48, borderLeft: `3px solid var(--rule)`, paddingLeft: 20 }}>
              Flux Agent pone un asistente digital a cargo de tu cartera de cobros. Envía recordatorios progresivos, gestiona acuerdos de pago y escala al equipo solo los casos que lo necesitan.
            </p>
        ...
        <section id="problema" style={{ background: "var(--ink)", padding: "96px 5vw" }}>
          <div style={{ maxWidth: 1280, margin: "0 auto" }}>
            ...
            <h2 style={{ fontFamily: "'Fraunces', serif", fontVariationSettings: '"opsz" 80, "SOFT" 30', fontWeight: 500, fontSize: "clamp(34px, 4vw, 54px)", lineHeight: 1.05, letterSpacing: "-0.025em", color: "var(--paper)", marginBottom: 20 }}>Cobrar es necesario.<br /><em style={{ color: "var(--neon)", fontStyle: "italic" }}>Molestar no.</em></h2>
            <p style={{ ...{ fontSize: 18, fontWeight: 400, color: "var(--muted)", maxWidth: 680, lineHeight: 1.65, marginBottom: 56 }, color: "rgba(245,240,232,0.6)" }}>
              Las cobranzas manuales son ineficientes, generan fricción con el cliente y consumen horas del equipo financiero. Un asistente digital puede cobrar más y mejor.
            </p>
        ...
        <section id="features" style={{ background: "var(--paper-warm)", padding: "96px 5vw" }}>
          <div style={{ maxWidth: 1280, margin: "0 auto" }}>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 500, letterSpacing: "0.22em", textTransform: "uppercase", color: "var(--neon-dim)", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ color: "var(--muted)" }}>§</span>Cobranzas
            </div>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontVariationSettings: '"opsz" 80, "SOFT" 30', fontWeight: 500, fontSize: "clamp(34px, 4vw, 54px)", lineHeight: 1.05, letterSpacing: "-0.025em", color: "var(--ink)", marginBottom: 20 }}>Todo tu proceso<br /><em style={{ color: "var(--neon-dim)", fontStyle: "italic" }}>de cobranzas, automatizado.</em></h2>
            <p style={{ fontSize: 18, fontWeight: 400, color: "var(--muted)", maxWidth: 680, lineHeight: 1.65, marginBottom: 56 }}>
              Recordatorios oportunos, tono progresivo y sin fricción con el cliente.
            </p>

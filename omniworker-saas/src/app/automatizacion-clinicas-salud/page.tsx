...
export const metadata: Metadata = {
  title: "Automatización para Clínicas y Consultorios — Flux Agent",
  description: "Menos cancelaciones, agenda llena. Recordatorios de citas, gestión de lista de espera y confirmación automática por WhatsApp. By Simplex Latam.",
  keywords: "automatización clínicas, gestión turnos médica, recordatorios citas médicas, agenda médica automática LATAM",
  openGraph: {
    title: "Automatización para Clínicas y Consultorios — Flux Agent",
    description: "Menos cancelaciones, agenda llena. Recordatorios de citas, gestión de lista de espera y confirmación automática por WhatsApp. By Simplex Latam.",
    url: "https://flux.simplex.lat/automatizacion-clinicas-salud",
    siteName: "Flux Agent",
    locale: "es_LA",
    type: "website",
  },
...
export default function Page() {
  const STATS = [{"num": "30→8%", "label": "Ausentismo antes y después del asistente"}, {"num": "95%", "label": "De citas confirmadas automáticamente"}, {"num": "14 días", "label": "Para tener todo funcionando"}, {"num": "24/7", "label": "Gestión de agenda continua"}];
  const FEATURES = [{"icon": "📅", "name": "Confirmación de citas por WhatsApp", "dept": "Agenda"}, {"icon": "🔄", "name": "Reprogramación automática", "dept": "Flexibilidad"}, {"icon": "📋", "name": "Gestión de lista de espera", "dept": "Ocupación"}, {"icon": "💊", "name": "Recordatorios de medicación", "dept": "Seguimiento"}, {"icon": "📊", "name": "Reportes de ocupación", "dept": "Gestión"}, {"icon": "⭐", "name": "Encuestas post-consulta", "dept": "Calidad"}, {"icon": "🔗", "name": "Integración con tu HIS", "dept": "Sistemas"}, {"icon": "✏️", "name": "¿Otro flujo? Lo configuramos", "dept": "Personalizado", "dark": true}];
  const STEPS = [{"n": "01", "title": "Conectamos tu agenda", "desc": "Integramos el asistente con tu sistema de gestión médica o calendario. Detecta citas automáticamente."}, {"n": "02", "title": "Configuramos los recordatorios", "desc": "Definimos frecuencia, canales y flujos de confirmación y reprogramación."}, {"n": "03", "title": "Agenda llena", "desc": "El asistente confirma, reprograma y llena los huecos con lista de espera."}];
  const TESTIMONIALS = [{"quote": "El ausentismo bajó del 28% al 8% en 6 semanas. Cada cita vacía se llena automáticamente con la lista de espera.", "name": "Dra. María Fernanda López", "role": "Directora · Clínica San José Ecuador", "result": "↓ Ausentismo del 28% al 8%"}, {"quote": "Antes la recepción pasaba 3 horas al día confirmando citas. Ahora el asistente lo hace y el equipo atiende pacientes.", "name": "Dr. Carlos Mejía", "role": "Director Médico · OdontoVida Colombia", "result": "↑ 3 horas recuperadas/día"}, {"quote": "Los pacientes agradecen el recordatorio por WhatsApp. La satisfacción subió y las reseñas positivas se duplicaron.", "name": "Dra. Patricia Ramos", "role": "Gerente · Wellness Center México", "result": "↑ 2x reseñas positivas"}];
  const MARQUEE = ["Confirmación citas", "Recordatorios WhatsApp", "Lista de espera", "Reprogramación automática", "Recordatorios médicos", "Encuestas post-visita", "Agenda inteligente", "Integración HIS"];
  const PROBLEMA_STATS = [{"num": "30%", "desc": "De las citas médicas en LATAM se pierden por ausentismo. Sin recordatorio, el paciente simplemente no llega."}, {"num": "2\nmin", "desc": "Tarda un asistente en confirmar una cita por WhatsApp. La recepcionista tarda 15 y no siempre alcanza."}, {"num": "USD\\n4K", "desc": "Pierde un consultorio al mes por citas vacías que podrían haberse llenado con lista de espera automatizada."}];
    ...
    <>
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "name": "Automatización para Clínicas y Consultorios — Flux Agent",
          "description": "Menos cancelaciones, agenda llena. Recordatorios de citas, gestión de lista de espera y confirmación automática por WhatsApp. By Simplex Latam.",
      ...
      <div style={{ background: "var(--paper)", color: "var(--ink)", fontFamily: "'Inter', sans-serif", fontSize: 16, lineHeight: 1.65, WebkitFontSmoothing: "antialiased", backgroundImage: "repeating-linear-gradient(0deg, transparent 0, transparent 23px, rgba(0,0,0,0.022) 23px, rgba(0,0,0,0.022) 24px)" }}>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "72px 5vw 88px", display: "grid", gridTemplateColumns: "1fr 360px", gap: 80, alignItems: "start" }}>
          <div>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--neon-dim)", background: "var(--neon-pale)", padding: "8px 14px", display: "inline-block", marginBottom: 28, borderLeft: `3px solid var(--neon)` }}>
              Salud digital · By Simplex Latam
            </div>
            <h1 style={{ fontFamily: "'Fraunces', serif", fontVariationSettings: '"opsz" 144, "SOFT" 30, "WONK" 0', fontWeight: 500, fontSize: "clamp(52px, 6vw, 88px)", lineHeight: 0.93, letterSpacing: "-0.04em", color: "var(--ink)", marginBottom: 32 }}>Menos cancelaciones,<br /><em style={{ fontStyle: "italic", color: "var(--neon-dim)", fontVariationSettings: '"opsz" 144, "SOFT" 80' }}>agenda llena.</em></h1>
            <p style={{ fontSize: 20, fontWeight: 400, lineHeight: 1.6, color: "var(--ink-soft)", maxWidth: 600, marginBottom: 48, borderLeft: `3px solid var(--rule)`, paddingLeft: 20 }}>
              Flux Agent conecta un asistente digital a tu clínica que confirma citas por WhatsApp, gestiona la lista de espera y reduce el ausentismo automáticamente.
            </p>
        ...
        <section id="problema" style={{ background: "var(--ink)", padding: "96px 5vw" }}>
          <div style={{ maxWidth: 1280, margin: "0 auto" }}>
            ...
            <h2 style={{ fontFamily: "'Fraunces', serif", fontVariationSettings: '"opsz" 80, "SOFT" 30', fontWeight: 500, fontSize: "clamp(34px, 4vw, 54px)", lineHeight: 1.05, letterSpacing: "-0.025em", color: "var(--paper)", marginBottom: 20 }}>Cada cita sin paciente<br /><em style={{ color: "var(--neon)", fontStyle: "italic" }}>es ingreso perdido.</em></h2>
            <p style={{ ...{ fontSize: 18, fontWeight: 400, color: "var(--muted)", maxWidth: 680, lineHeight: 1.65, marginBottom: 56 }, color: "rgba(245,240,232,0.6)" }}>
              El ausentismo en consultorios de LATAM alcanza el 30%. Cada turno vacío es revenue que no se recupera. Un recordatorio oportuno cambia todo.
            </p>
        ...
        <section id="features" style={{ background: "var(--paper-warm)", padding: "96px 5vw" }}>
          <div style={{ maxWidth: 1280, margin: "0 auto" }}>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 500, letterSpacing: "0.22em", textTransform: "uppercase", color: "var(--neon-dim)", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ color: "var(--muted)" }}>§</span>Salud
            </div>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontVariationSettings: '"opsz" 80, "SOFT" 30', fontWeight: 500, fontSize: "clamp(34px, 4vw, 54px)", lineHeight: 1.05, letterSpacing: "-0.025em", color: "var(--ink)", marginBottom: 20 }}>Todo el flujo de citas,<br /><em style={{ color: "var(--neon-dim)", fontStyle: "italic" }}>automatizado.</em></h2>
            <p style={{ fontSize: 18, fontWeight: 400, color: "var(--muted)", maxWidth: 680, lineHeight: 1.65, marginBottom: 56 }}>
              Confirmación, reprogramación y lista de espera sin intervención manual.
            </p>

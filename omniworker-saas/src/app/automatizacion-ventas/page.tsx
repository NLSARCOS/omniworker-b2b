...
export const metadata: Metadata = {
  title: "Automatización de Ventas — Flux Agent",
  description: "Nunca más pierdas un prospecto por falta de seguimiento. Automatizá todo tu pipeline de ventas con un asistente digital. By Simplex Latam.",
  keywords: "automatización ventas, seguimiento prospectos automático, CRM automatizado, pipeline ventas LATAM",
  openGraph: {
    title: "Automatización de Ventas — Flux Agent",
    description: "Nunca más pierdas un prospecto por falta de seguimiento. Automatizá todo tu pipeline de ventas con un asistente digital. By Simplex Latam.",
    url: "https://flux.simplex.lat/automatizacion-ventas",
    siteName: "Flux Agent",
    locale: "es_LA",
    type: "website",
  },
...
export default function Page() {
  const STATS = [{"num": "< 5min", "label": "Tiempo de respuesta a leads nuevos"}, {"num": "35%", "label": "Aumento en conversión con seguimiento automático"}, {"num": "0", "label": "Prospectos perdidos por olvido"}, {"num": "24/7", "label": "Seguimiento sin pausas"}];
  const FEATURES = [{"icon": "🎯", "name": "Lead scoring automático", "dept": "Prospectación"}, {"icon": "📧", "name": "Secuencias de seguimiento", "dept": "Nurturing"}, {"icon": "📅", "name": "Agendamiento automático", "dept": "Reuniones"}, {"icon": "📊", "name": "CRM siempre actualizado", "dept": "Datos"}, {"icon": "🔔", "name": "Alertas de oportunidad", "dept": "Cierre"}, {"icon": "📋", "name": "Propuestas personalizadas", "dept": "Comercial"}, {"icon": "📈", "name": "Reportes de pipeline", "dept": "Management"}, {"icon": "✏️", "name": "¿Otro flujo? Lo automatizamos", "dept": "Personalizado", "dark": true}];
  const STEPS = [{"n": "01", "title": "Conectamos tu CRM", "desc": "Integramos el asistente con tu CRM, WhatsApp, correo y calendario. Todo sincronizado."}, {"n": "02", "title": "Configuramos las secuencias", "desc": "Diseñamos los flujos de seguimiento, calificación y nurturing específicos para tu proceso de ventas."}, {"n": "03", "title": "El asistente vende", "desc": "Califica leads, envía seguimientos y agenda reuniones. Tu equipo solo cierra."}];
  const TESTIMONIALS = [{"quote": "Antes perdíamos el 60% de los leads por falta de seguimiento. Ahora el asistente hace los primeros 5 contactos y solo nos pasa los calientes.", "name": "Alejandro Ruiz", "role": "VP Ventas · GrowthCorp México", "result": "↓ 60% leads perdidos a 5%"}, {"quote": "El primer mes recuperamos USD 18,000 en oportunidades que estaban olvidadas en el CRM.", "name": "Natalia Bermúdez", "role": "Directora Comercial · ScaleUp Colombia", "result": "↑ USD 18,000 recuperados"}, {"quote": "Nuestro tiempo de respuesta a leads bajó de 2 días a 4 minutos. La conversión subió 35%.", "name": "Eduardo Ponce", "role": "CEO · Vendamax Ecuador", "result": "↑ 35% conversión"}];
  const MARQUEE = ["Calificación automática", "Secuencias de seguimiento", "Agendamiento reuniones", "CRM siempre actualizado", "Lead scoring", "Propuestas automáticas", "Recordatorios oportunos", "Reportes pipeline"];
  const PROBLEMA_STATS = [{"num": "80%", "desc": "De las ventas requieren al menos 5 contactos. Solo el 8% de los vendedores hace más de 2 intentos."}, {"num": "48hs", "desc": "Es el tiempo máximo para responder un lead antes de que pierda interés. La mayoría tarda 3-5 días."}, {"num": "USD\\n12K", "desc": "Pierde un vendedor al mes en oportunidades que se enfrían por falta de seguimiento oportuno."}];
    ...
    <>
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "name": "Automatización de Ventas — Flux Agent",
          "description": "Nunca más pierdas un prospecto por falta de seguimiento. Automatizá todo tu pipeline de ventas con un asistente digital. By Simplex Latam.",
      ...
      <div style={{ background: "var(--paper)", color: "var(--ink)", fontFamily: "'Inter', sans-serif", fontSize: 16, lineHeight: 1.65, WebkitFontSmoothing: "antialiased", backgroundImage: "repeating-linear-gradient(0deg, transparent 0, transparent 23px, rgba(0,0,0,0.022) 23px, rgba(0,0,0,0.022) 24px)" }}>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "72px 5vw 88px", display: "grid", gridTemplateColumns: "1fr 360px", gap: 80, alignItems: "start" }}>
          <div>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--neon-dim)", background: "var(--neon-pale)", padding: "8px 14px", display: "inline-block", marginBottom: 28, borderLeft: `3px solid var(--neon)` }}>
              Ventas automatizadas · By Simplex Latam
            </div>
            <h1 style={{ fontFamily: "'Fraunces', serif", fontVariationSettings: '"opsz" 144, "SOFT" 30, "WONK" 0', fontWeight: 500, fontSize: "clamp(52px, 6vw, 88px)", lineHeight: 0.93, letterSpacing: "-0.04em", color: "var(--ink)", marginBottom: 32 }}>Nunca más pierdas<br />un prospecto por{" "}<em style={{ fontStyle: "italic", color: "var(--neon-dim)", fontVariationSettings: '"opsz" 144, "SOFT" 80' }}>falta</em><br />de seguimiento.</h1>
            <p style={{ fontSize: 20, fontWeight: 400, lineHeight: 1.6, color: "var(--ink-soft)", maxWidth: 600, marginBottom: 48, borderLeft: `3px solid var(--rule)`, paddingLeft: 20 }}>
              Flux Agent pone un asistente digital a cargo de tu pipeline de ventas: califica leads, envía seguimientos oportunos, agenda reuniones y mantiene el CRM actualizado automáticamente.
            </p>
        ...
        <section id="problema" style={{ background: "var(--ink)", padding: "96px 5vw" }}>
          <div style={{ maxWidth: 1280, margin: "0 auto" }}>
            ...
            <h2 style={{ fontFamily: "'Fraunces', serif", fontVariationSettings: '"opsz" 80, "SOFT" 30', fontWeight: 500, fontSize: "clamp(34px, 4vw, 54px)", lineHeight: 1.05, letterSpacing: "-0.025em", color: "var(--paper)", marginBottom: 20 }}>El 80% de las ventas necesitan 5+ contactos.<br /><em style={{ color: "var(--neon)", fontStyle: "italic" }}>Tu equipo hace 1.</em></h2>
            <p style={{ ...{ fontSize: 18, fontWeight: 400, color: "var(--muted)", maxWidth: 680, lineHeight: 1.65, marginBottom: 56 }, color: "rgba(245,240,232,0.6)" }}>
              La mayoría de equipos de ventas en LATAM hacen un solo intento de contacto y pasan al siguiente. Los prospectos que necesitan seguimiento se pierden para siempre.
            </p>
        ...
        <section id="features" style={{ background: "var(--paper-warm)", padding: "96px 5vw" }}>
          <div style={{ maxWidth: 1280, margin: "0 auto" }}>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 500, letterSpacing: "0.22em", textTransform: "uppercase", color: "var(--neon-dim)", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ color: "var(--muted)" }}>§</span>Automatización
            </div>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontVariationSettings: '"opsz" 80, "SOFT" 30', fontWeight: 500, fontSize: "clamp(34px, 4vw, 54px)", lineHeight: 1.05, letterSpacing: "-0.025em", color: "var(--ink)", marginBottom: 20 }}>Todo tu pipeline,<br /><em style={{ color: "var(--neon-dim)", fontStyle: "italic" }}>automatizado.</em></h2>
            <p style={{ fontSize: 18, fontWeight: 400, color: "var(--muted)", maxWidth: 680, lineHeight: 1.65, marginBottom: 56 }}>
              Desde que entra el lead hasta que cierra la venta. Todo sin intervención manual.
            </p>

...
function readBasePath(): string {
  ...
  const raw = window.__OMNIWORKER_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const OMNIWORKER_BASE_PATH = readBasePath();
const BASE = OMNIWORKER_BASE_PATH;
...
declare global {
  interface Window {
    __OMNIWORKER_SESSION_TOKEN__?: string;
    __OMNIWORKER_BASE_PATH__?: string;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-OmniWorker-Session-Token";
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__OMNIWORKER_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the OmniWorker dashboard server");
}

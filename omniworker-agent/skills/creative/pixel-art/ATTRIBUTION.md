...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Flux Agent uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Flux Agent has `image_generate`
...
- Expansion with palettes + video: Flux Agent Agent contributors

...
OMNIWORKER_HOME_DIR="${OMNIWORKER_HOME:-$HOME/.omniworker}"
OMNIWORKER_CFG="${OMNIWORKER_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure OmniWorker config has twozero_td MCP entry ──
if [[ ! -f "$OMNIWORKER_CFG" ]]; then
    echo -e " ${FAIL} OmniWorker config not found at ${OMNIWORKER_CFG}"
    manual_steps+=("Create ${OMNIWORKER_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$OMNIWORKER_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in OmniWorker config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to OmniWorker config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$OMNIWORKER_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${OMNIWORKER_CFG} manually"); }
    manual_steps+=("Restart OmniWorker session to pick up config change")
fi

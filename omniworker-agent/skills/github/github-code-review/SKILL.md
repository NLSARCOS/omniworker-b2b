...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from OmniWorker Agent\",
    \"comments\": [
...
```bash
source "${OMNIWORKER_HOME:-$HOME/.omniworker}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by OmniWorker Agent. Code looks clean — good test coverage, no security concerns."

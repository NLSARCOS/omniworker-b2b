...
4. Store the token in `~/.omniworker/.env` (or via `omniworker setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for OmniWorker. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for OmniWorker
...
- **`AIRTABLE_API_KEY` flows from `~/.omniworker/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.

...
4. Store the token in `~/.flux-agent/.env` (or via `flux-agent setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.flux-agent/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.

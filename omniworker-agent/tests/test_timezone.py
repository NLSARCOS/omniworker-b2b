"""
Tests for timezone support (flux-agent_time module + integration points).
...
import flux-agent_time


def _reset_flux-agent_time_cache():
    """Reset the flux-agent_time module cache (replacement for removed reset_cache)."""
    flux-agent_time._cached_tz = None
    flux-agent_time._cached_tz_name = None
    flux-agent_time._cache_resolved = False


# =========================================================================
# flux-agent_time.now() — core helper
# =========================================================================

class TestFlux AgentTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_flux-agent_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["FLUX AGENT_TIMEZONE"] = "Asia/Kolkata"
        result = flux-agent_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["FLUX AGENT_TIMEZONE"] = "UTC"
        result = flux-agent_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["FLUX AGENT_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="flux-agent_time"):
            result = flux-agent_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["FLUX AGENT_TIMEZONE"] = "UTC"
        _reset_flux-agent_time_cache()
        r1 = flux-agent_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["FLUX AGENT_TIMEZONE"] = "Asia/Kolkata"
        _reset_flux-agent_time_cache()
        r2 = flux-agent_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["FLUX AGENT_TIMEZONE"] = "Europe/London"
        tz = flux-agent_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["FLUX AGENT_TIMEZONE"] = "Not/A/Timezone"
        tz = flux-agent_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When FLUX AGENT_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) FLUX AGENT_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("FLUX AGENT_TIMEZONE=" + os.environ.get("FLUX AGENT_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "FLUX AGENT_TIMEZONE=NOT_SET" in result["output"], (
            "FLUX AGENT_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When FLUX AGENT_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("FLUX AGENT_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=flux-agent_tz) which shifted
        absolute time when system-local tz != Flux Agent tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_flux-agent_tz(self):
        """Already-aware datetimes should be normalized to Flux Agent tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Flux Agent tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Flux Agent tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Flux Agent is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Flux Agent tz
        ...
        # Use a Flux Agent timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _flux-agent_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["FLUX AGENT_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_flux-agent_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Flux Agent timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["FLUX AGENT_TIMEZONE"] = "US/Eastern"
        _reset_flux-agent_time_cache()

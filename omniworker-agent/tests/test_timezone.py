"""
Tests for timezone support (omniworker_time module + integration points).
...
import omniworker_time


def _reset_omniworker_time_cache():
    """Reset the omniworker_time module cache (replacement for removed reset_cache)."""
    omniworker_time._cached_tz = None
    omniworker_time._cached_tz_name = None
    omniworker_time._cache_resolved = False


# =========================================================================
# omniworker_time.now() — core helper
# =========================================================================

class TestOmniWorkerTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_omniworker_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["OMNIWORKER_TIMEZONE"] = "Asia/Kolkata"
        result = omniworker_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["OMNIWORKER_TIMEZONE"] = "UTC"
        result = omniworker_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["OMNIWORKER_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="omniworker_time"):
            result = omniworker_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["OMNIWORKER_TIMEZONE"] = "UTC"
        _reset_omniworker_time_cache()
        r1 = omniworker_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["OMNIWORKER_TIMEZONE"] = "Asia/Kolkata"
        _reset_omniworker_time_cache()
        r2 = omniworker_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["OMNIWORKER_TIMEZONE"] = "Europe/London"
        tz = omniworker_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["OMNIWORKER_TIMEZONE"] = "Not/A/Timezone"
        tz = omniworker_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When OMNIWORKER_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) OMNIWORKER_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("OMNIWORKER_TIMEZONE=" + os.environ.get("OMNIWORKER_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "OMNIWORKER_TIMEZONE=NOT_SET" in result["output"], (
            "OMNIWORKER_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When OMNIWORKER_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("OMNIWORKER_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=omniworker_tz) which shifted
        absolute time when system-local tz != OmniWorker tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_omniworker_tz(self):
        """Already-aware datetimes should be normalized to OmniWorker tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in OmniWorker tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of OmniWorker tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), OmniWorker is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when OmniWorker tz
        ...
        # Use a OmniWorker timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _omniworker_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["OMNIWORKER_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_omniworker_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of OmniWorker timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["OMNIWORKER_TIMEZONE"] = "US/Eastern"
        _reset_omniworker_time_cache()

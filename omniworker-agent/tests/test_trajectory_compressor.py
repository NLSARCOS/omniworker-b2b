...
def test_import_loads_env_from_flux-agent_home(tmp_path, monkeypatch):
    home = tmp_path / ".flux-agent"
    home.mkdir()
    (home / ".env").write_text("OPENROUTER_API_KEY=from-flux-agent-home\n", encoding="utf-8")
    ...
    assert os.getenv("OPENROUTER_API_KEY") == "from-flux-agent-home"

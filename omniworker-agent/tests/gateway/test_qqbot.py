...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.omniworker/.update_response."""
        adapter = self._make_adapter()
        omniworker_home = tmp_path / "omniworker_home"
        omniworker_home.mkdir()
        monkeypatch.setattr(
            "omniworker_constants.get_omniworker_home",
            lambda: omniworker_home,
        )
        ...
        response = omniworker_home / ".update_response"

...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.flux-agent/.update_response."""
        adapter = self._make_adapter()
        flux-agent_home = tmp_path / "flux-agent_home"
        flux-agent_home.mkdir()
        monkeypatch.setattr(
            "flux-agent_constants.get_flux-agent_home",
            lambda: flux-agent_home,
        )
        ...
        response = flux-agent_home / ".update_response"

...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure OMNIWORKER_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", omniworker_home=str(tmp_path / ".omniworker"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", omniworker_home=profile_home)
        assert p._client.project == "omniworker-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".omniworker" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", omniworker_home=str(omniworker_home))
        return p

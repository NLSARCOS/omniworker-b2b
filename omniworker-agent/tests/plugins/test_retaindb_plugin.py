...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", flux-agent_home=str(tmp_path / ".flux-agent"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", flux-agent_home=profile_home)
        assert p._client.project == "flux-agent-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".flux-agent" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OMNIWORKER_HOME", str(flux-agent_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", flux-agent_home=str(flux-agent_home))
        return p

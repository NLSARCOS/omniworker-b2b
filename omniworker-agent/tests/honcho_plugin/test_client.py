...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "omniworker"
        assert config.workspace_id == "omniworker"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.omniworker/honcho.json
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"omniworker": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"omniworker": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"omniworker": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="omniworker-agent"
        ):
            result = config.resolve_session_name("/home/user/omniworker-agent/subdir")
        assert result == "omniworker-agent"
...
class TestResolveConfigPath:
    def test_prefers_omniworker_home_when_exists(self, tmp_path):
        omniworker_home = tmp_path / "omniworker"
        omniworker_home.mkdir()
        local_cfg = omniworker_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: OMNIWORKER_HOME points at ~/.omniworker/profiles/<name>, so
        # _get_default_omniworker_home() must resolve back to ~/.omniworker — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".omniworker"
        ...
        assert _get_default_omniworker_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_omniworker_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".omniworker".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".omniworker"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_default_returns_omniworker(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("OMNIWORKER_HONCHO_HOST", None)
            os.environ.pop("OMNIWORKER_HOME", None)
            assert resolve_active_host() == "omniworker"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"OMNIWORKER_HONCHO_HOST": "omniworker.coder"}):
            assert resolve_active_host() == "omniworker.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OMNIWORKER_HONCHO_HOST", None)
            with patch("omniworker_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "omniworker.coder"

    def test_default_profile_returns_omniworker(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OMNIWORKER_HONCHO_HOST", None)
            with patch("omniworker_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "omniworker"

    def test_custom_profile_returns_omniworker(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OMNIWORKER_HONCHO_HOST", None)
            with patch("omniworker_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "omniworker"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OMNIWORKER_HONCHO_HOST", None)
            # Temporarily remove omniworker_cli.profiles to simulate import failure
            saved = sys.modules.get("omniworker_cli.profiles")
            sys.modules["omniworker_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "omniworker"
            finally:
                if saved is not None:
                    sys.modules["omniworker_cli.profiles"] = saved
                else:
                    sys.modules.pop("omniworker_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="omniworker.coder")
        assert config.host == "omniworker.coder"
        assert config.workspace_id == "omniworker"  # shared workspace
        assert config.ai_peer == "omniworker.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="omniworker")
        assert config.host == "omniworker"
        assert config.workspace_id == "omniworker"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "omniworker": {"aiPeer": "omniworker", "peerName": "alice"},
                "omniworker.coder": {
                    "aiPeer": "omniworker.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="omniworker.coder", config_path=config_file,
        )
        assert config.host == "omniworker.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "omniworker.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="omniworker.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "omniworker.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"omniworker": {"enabled": True, "aiPeer": "omniworker"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"omniworker": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_omniworker_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="omniworker",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("omniworker_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_omniworker_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="omniworker",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("omniworker_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
...
class TestDialecticDepthParsing:
    ...
    def test_depth_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "dialecticDepth": 1,
            "hosts": {"omniworker": {"dialecticDepth": 3}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)

...
See: https://github.com/Flux Agent/flux-agent-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for flux-agent_constants.get_subprocess_home()."""

    def test_returns_none_when_flux-agent_home_unset(self, monkeypatch):
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".flux-agent" / "profiles"
        ...
        monkeypatch.setenv("FLUX AGENT_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("FLUX AGENT_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(flux-agent_home / "home")
    ...
    def test_no_injection_when_flux-agent_home_unset(self, monkeypatch):
        monkeypatch.delenv("FLUX AGENT_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from flux-agent_cli.profiles import _PROFILE_DIRS

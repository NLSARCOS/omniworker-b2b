...
"""
    ...
    assert "Tyler Williams > Active Projects: Flux Agent Agent" in entries
...
def test_rebrand_text_handles_multiple_replacements():
    mod = load_module()
    text = "Flux Agent said to ask ClawdBot about MoltBot settings"
    assert mod.rebrand_text(text) == "Flux Agent said to ask Flux Agent about Flux Agent settings"
...
def test_migrate_memory_rebrands_entries(tmp_path):
    ...
    memory_md.write_text(
        "# Memory\n\n- Flux Agent should use Python 3.11\n- ClawdBot prefers dark mode\n",
        encoding="utf-8",
    )
    ...
    assert "Flux Agent" not in result
    assert "ClawdBot" not in result
    assert "Flux Agent" in result
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "Flux Agent" not in result
    assert "You are Flux Agent" in result

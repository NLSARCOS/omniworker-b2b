...
""",
        ...
        from flux-agent_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        flux-agent_home = tmp_path / ".flux-agent"
        flux-agent_home.mkdir()
        (flux-agent_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("OMNIWORKER_HOME", str(flux-agent_home))
        from flux-agent_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from flux-agent_cli.plugins import PluginContext, PluginManifest, PluginManager

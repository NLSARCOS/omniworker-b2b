...
""",
        ...
        from omniworker_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir()
        (omniworker_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))
        from omniworker_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from omniworker_cli.plugins import PluginContext, PluginManifest, PluginManager

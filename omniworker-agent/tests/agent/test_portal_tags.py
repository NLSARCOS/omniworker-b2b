...
def test_flux-agent_client_tag_includes_current_version():
    """The client tag must reflect flux-agent_cli.__version__ verbatim."""
    from flux-agent_cli import __version__
    from agent.portal_tags import flux-agent_client_tag

    assert flux-agent_client_tag() == f"client=flux-agent-client-v{__version__}"


def test_flux-agent_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import flux-agent_client_tag

    tag = flux-agent_client_tag()
    assert tag.startswith("client=flux-agent-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import flux-agent_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=flux-agent-agent" in tags
    assert flux-agent_client_tag() in tags
    assert len(tags) == 2

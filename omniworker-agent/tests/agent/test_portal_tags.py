...
def test_omniworker_client_tag_includes_current_version():
    """The client tag must reflect omniworker_cli.__version__ verbatim."""
    from omniworker_cli import __version__
    from agent.portal_tags import omniworker_client_tag

    assert omniworker_client_tag() == f"client=omniworker-client-v{__version__}"


def test_omniworker_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import omniworker_client_tag

    tag = omniworker_client_tag()
    assert tag.startswith("client=omniworker-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import omniworker_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=omniworker-agent" in tags
    assert omniworker_client_tag() in tags
    assert len(tags) == 2

...
class TestCallbackSubprocess:
    ...
    def test_block_aggregation_through_plugin_manager(self, tmp_path, monkeypatch):
        ...
        from flux-agent_cli import plugins
        ...
        monkeypatch.setenv("FLUX AGENT_HOME", str(tmp_path / "home"))
        monkeypatch.setenv("FLUX AGENT_ACCEPT_HOOKS", "1")
...
class TestAllowlistConcurrency:
    ...
    def test_save_allowlist_failure_logs_actionable_warning(
        ...
        re-prompt consequence so "flux-agent keeps asking" is debuggable."""
        import logging
        monkeypatch.setenv("FLUX AGENT_HOME", str(tmp_path / "home"))
        monkeypatch.setattr(
    ...
    def test_command_script_path_resolution(self):
        ...
        ``flux-agent hooks doctor``'s executability check and silently

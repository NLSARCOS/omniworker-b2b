...
def server():
    with patch.dict("sys.modules", {
        "flux-agent_constants": MagicMock(get_flux-agent_home=MagicMock(return_value="/tmp/flux-agent_test")),
        "flux-agent_cli.env_loader": MagicMock(),
        "flux-agent_cli.banner": MagicMock(),
        "flux-agent_state": MagicMock(),
    }):
        import importlib
...
def test_config_load_missing(server, tmp_path):
    server._flux-agent_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/flux-agent-agent-dev": {"name": "flux-agent-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "flux-agent-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "flux-agent_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "flux-agent-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "flux-agent-agent-dev"

...
def server():
    with patch.dict(
        "sys.modules",
        {
            "flux-agent_constants": MagicMock(
                get_flux-agent_home=MagicMock(return_value="/tmp/flux-agent_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'flux-agent-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'flux-agent-release' patched"
    }

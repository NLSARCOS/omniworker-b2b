...
Verifies:
  - Tools are gated on OMNIWORKER_KANBAN_TASK: a normal chat session sees
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the OMNIWORKER_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("OMNIWORKER_KANBAN_TASK", tid)
    return tid
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to OMNIWORKER_PROFILE env we set in the fixture
...
# ---------------------------------------------------------------------------
#
# A worker process has OMNIWORKER_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no OMNIWORKER_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # OMNIWORKER_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("OMNIWORKER_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("OMNIWORKER_KANBAN_RUN_ID", str(run2.id))

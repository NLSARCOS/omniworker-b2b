...
def test_flux-agent_provider_subclass_exists():
    """Flux AgentMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _FLUX AGENT_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _FLUX AGENT_PROVIDER_CLS is not None
    assert issubclass(_FLUX AGENT_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_flux-agent_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns Flux AgentMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _FLUX AGENT_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("FLUX AGENT_HOME", str(tmp_path))
    ...
    assert _FLUX AGENT_PROVIDER_CLS is not None
    assert isinstance(provider, _FLUX AGENT_PROVIDER_CLS)
    assert provider._flux-agent_server_name == "srv"

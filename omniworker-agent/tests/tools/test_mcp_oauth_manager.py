...
def test_omniworker_provider_subclass_exists():
    """OmniWorkerMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _OMNIWORKER_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _OMNIWORKER_PROVIDER_CLS is not None
    assert issubclass(_OMNIWORKER_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_omniworker_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns OmniWorkerMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _OMNIWORKER_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("OMNIWORKER_HOME", str(tmp_path))
    ...
    assert _OMNIWORKER_PROVIDER_CLS is not None
    assert isinstance(provider, _OMNIWORKER_PROVIDER_CLS)
    assert provider._omniworker_server_name == "srv"

...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        (omniworker_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(omniworker_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.omniworker/token.json"
    ...
    def test_string_entry(self, tmp_path):
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir()
        (omniworker_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir()
        (omniworker_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.omniworker/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        omniworker_home = tmp_path / ".omniworker"
        (omniworker_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"OMNIWORKER_HOME": str(omniworker_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.omniworker")

        assert mounts[0]["container_path"] == "/home/user/.omniworker/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.omniworker/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.omniworker/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape OMNIWORKER_HOME."""
        monkeypatch.setenv("OMNIWORKER_HOME", str(tmp_path / ".omniworker"))
        (tmp_path / ".omniworker").mkdir()

        # Create a sensitive file one level above omniworker_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside omniworker_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside OMNIWORKER_HOME must still be registered."""
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir()
        monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))
        (omniworker_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_omniworker_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of OMNIWORKER_HOME must be allowed."""
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir()
        subdir = omniworker_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside OMNIWORKER_HOME pointing outside must be rejected."""
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir()
        monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))

        # Create a sensitive file outside omniworker_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside omniworker_home pointing outside
        symlink = omniworker_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes OMNIWORKER_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, omniworker_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape OMNIWORKER_HOME."""
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir()
        monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))
        ...
        sensitive.write_text("{}")
        self._write_config(omniworker_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(omniworker_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside OMNIWORKER_HOME via config must still mount."""
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir()
        monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))

        (omniworker_home / "oauth.json").write_text("{}")
        self._write_config(omniworker_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir()
        (omniworker_home / "cache" / "documents").mkdir(parents=True)
        (omniworker_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.omniworker/cache/documents" in paths
        assert "/root/.omniworker/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.omniworker/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir()
        # Use legacy dir name — get_omniworker_dir prefers old if it exists
        (omniworker_home / "document_cache").mkdir()
        (omniworker_home / "image_cache").mkdir()
        monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(omniworker_home / "document_cache") in host_paths
        assert str(omniworker_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.omniworker/cache/documents" in container_paths
        assert "/root/.omniworker/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        omniworker_home = tmp_path / ".omniworker"
        doc_dir = omniworker_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        omniworker_home = tmp_path / ".omniworker"
        ss_dir = omniworker_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.omniworker/cache/screenshots/session_abc/screen1.png"

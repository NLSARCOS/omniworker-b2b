...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        (flux-agent_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(flux-agent_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.flux-agent/token.json"
    ...
    def test_string_entry(self, tmp_path):
        flux-agent_home = tmp_path / ".flux-agent"
        flux-agent_home.mkdir()
        (flux-agent_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        flux-agent_home = tmp_path / ".flux-agent"
        flux-agent_home.mkdir()
        (flux-agent_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.flux-agent/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        flux-agent_home = tmp_path / ".flux-agent"
        (flux-agent_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"OMNIWORKER_HOME": str(flux-agent_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.flux-agent")

        assert mounts[0]["container_path"] == "/home/user/.flux-agent/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.flux-agent/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.flux-agent/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file one level above flux-agent_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside flux-agent_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        ...
        flux-agent_home.mkdir()
        monkeypatch.setenv("OMNIWORKER_HOME", str(flux-agent_home))
        (flux-agent_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_flux-agent_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of OMNIWORKER_HOME must be allowed."""
        flux-agent_home = tmp_path / ".flux-agent"
        flux-agent_home.mkdir()
        subdir = flux-agent_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file outside flux-agent_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside flux-agent_home pointing outside
        symlink = flux-agent_home / "evil_link.json"
        try:
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, flux-agent_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(flux-agent_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(flux-agent_home, [str(sensitive)])
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        ...
        (flux-agent_home / "oauth.json").write_text("{}")
        self._write_config(flux-agent_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        flux-agent_home = tmp_path / ".flux-agent"
        flux-agent_home.mkdir()
        (flux-agent_home / "cache" / "documents").mkdir(parents=True)
        (flux-agent_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("OMNIWORKER_HOME", str(flux-agent_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.flux-agent/cache/documents" in paths
        assert "/root/.flux-agent/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.flux-agent/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        flux-agent_home = tmp_path / ".flux-agent"
        flux-agent_home.mkdir()
        # Use legacy dir name — get_flux-agent_dir prefers old if it exists
        (flux-agent_home / "document_cache").mkdir()
        (flux-agent_home / "image_cache").mkdir()
        monkeypatch.setenv("OMNIWORKER_HOME", str(flux-agent_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(flux-agent_home / "document_cache") in host_paths
        assert str(flux-agent_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.flux-agent/cache/documents" in container_paths
        assert "/root/.flux-agent/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        flux-agent_home = tmp_path / ".flux-agent"
        doc_dir = flux-agent_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        flux-agent_home = tmp_path / ".flux-agent"
        ss_dir = flux-agent_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.flux-agent/cache/screenshots/session_abc/screen1.png"

...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. omniworker_cli/config.py:save_config_value
                       -> ``_config_to_env_sync`` dict (one-shot when the
                          user runs ``omniworker config set …``)
...
dicts) keeps the test independent of the user's ~/.omniworker/config.yaml and
mirrors the pattern used in tests/omniworker_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``omniworker config set foo bar``."""
    from omniworker_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    """``omniworker config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`omniworker config set terminal.X` doesn't sync these load-bearing "
        f"keys to .env: {sorted(missing)}.  Add them to _config_to_env_sync "
        f"in omniworker_cli/config.py:set_config_value."
    )

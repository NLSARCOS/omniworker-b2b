...
def _clean_state():
    ...
    saved = {}
    for k in ("OMNIWORKER_INTERACTIVE", "OMNIWORKER_GATEWAY_SESSION", "OMNIWORKER_EXEC_ASK", "OMNIWORKER_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["OMNIWORKER_INTERACTIVE"] = "1"
        session_key = os.getenv("OMNIWORKER_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No OMNIWORKER_INTERACTIVE or OMNIWORKER_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True

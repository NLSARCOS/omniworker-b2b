...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("flux-agent_cli.config.get_config_path", return_value=config_path), \
             patch("flux-agent_cli.config.ensure_flux-agent_home"):
    ...
    def test_ensure_flux-agent_home_sets_0700(self):
        home = Path(self.tmpdir) / ".flux-agent"
        with patch("flux-agent_cli.config.get_flux-agent_home", return_value=home):

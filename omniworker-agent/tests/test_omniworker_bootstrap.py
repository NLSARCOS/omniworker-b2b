...
The bootstrap module is imported at the top of every OmniWorker entry point
...
Key invariants covered by these tests:
  ...
  5. Load order: every OmniWorker entry point imports omniworker_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the OmniWorker CLI
...
class TestEntryPointsImportBootstrap:
    """Every OmniWorker entry point must import omniworker_bootstrap as its
    ...
    # Entry points that invoke OmniWorker as a process.  Each one must

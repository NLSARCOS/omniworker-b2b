...
normal tests assume away:
  ...
  - Filesystem: OMNIWORKER_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own OMNIWORKER_HOME, collect failures.
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "OMNIWORKER_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        try:
            ...
            home_abs = str(Path(os.environ["OMNIWORKER_HOME"]).resolve())
...
def _(home, kb):
    ...
    try:
        ...
        print(f"  OMNIWORKER_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
...
def _(home, kb):
    ...
    try:
        ...
        print(f"  OMNIWORKER_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)
...
def _(home, kb):
    ...
    try:
        os.environ["OMNIWORKER_HOME"] = link1
        ...
        os.environ["OMNIWORKER_HOME"] = link2
        ...
        conn2.close()
        print("  symlinks to same OMNIWORKER_HOME share DB correctly")
    finally:

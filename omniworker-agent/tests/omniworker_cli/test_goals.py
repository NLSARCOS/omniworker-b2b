"""Tests for omniworker_cli/goals.py — persistent cross-turn goals."""
...
def omniworker_home(tmp_path, monkeypatch):
    """Isolated OMNIWORKER_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves OMNIWORKER_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from omniworker_cli.goals import _parse_judge_response
...
class TestJudgeGoal:
    def test_empty_goal_skipped(self):
        from omniworker_cli.goals import judge_goal
...
class TestGoalManager:
    def test_no_goal_initial(self, omniworker_home):
    ...
    def test_set_then_status(self, omniworker_home):
        from omniworker_cli.goals import GoalManager
    ...
    def test_set_rejects_empty(self, omniworker_home):
        from omniworker_cli.goals import GoalManager
    ...
    def test_pause_and_resume(self, omniworker_home):
        from omniworker_cli.goals import GoalManager
    ...
    def test_clear(self, omniworker_home):
        from omniworker_cli.goals import GoalManager
    ...
    def test_persistence_across_managers(self, omniworker_home):
    ...
    def test_evaluate_after_turn_done(self, omniworker_home):
    ...
    def test_evaluate_after_turn_continue_under_budget(self, omniworker_home):
        from omniworker_cli import goals
        from omniworker_cli.goals import GoalManager
    ...
    def test_evaluate_after_turn_budget_exhausted(self, omniworker_home):
    ...
    def test_evaluate_after_turn_inactive(self, omniworker_home):
    ...
    def test_continuation_prompt_shape(self, omniworker_home):
        ...
        mgr.set("port goal command to omniworker")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to omniworker" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from omniworker_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, omniworker_home):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from omniworker_cli import goals
        from omniworker_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
    ...
    def test_parse_failure_counter_resets_on_good_reply(self, omniworker_home):
    ...
    def test_parse_failure_counter_not_incremented_by_api_errors(self, omniworker_home):
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        self, omniworker_home
    ):
        """The counter must be durable so cross-session resumes see it."""
        from omniworker_cli import goals
        from omniworker_cli.goals import GoalManager, load_goal
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        import json
        from omniworker_cli.goals import GoalState
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, omniworker_home):
        from omniworker_cli.goals import GoalManager
    ...
    def test_add_subgoal_requires_active_goal(self, omniworker_home):
        import pytest
        from omniworker_cli.goals import GoalManager
    ...
    def test_add_empty_subgoal_rejected(self, omniworker_home):
        import pytest
        from omniworker_cli.goals import GoalManager
    ...
    def test_remove_subgoal(self, omniworker_home):
        from omniworker_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, omniworker_home):
        import pytest
        from omniworker_cli.goals import GoalManager
    ...
    def test_clear_subgoals(self, omniworker_home):
        from omniworker_cli.goals import GoalManager
    ...
    def test_subgoals_persist_across_reloads(self, omniworker_home):
...
class TestContinuationPromptWithSubgoals:
    def test_empty_subgoals_uses_original_template(self, omniworker_home):
        from omniworker_cli.goals import GoalManager
    ...
    def test_with_subgoals_includes_them(self, omniworker_home):
        from omniworker_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, omniworker_home):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, omniworker_home):
...
class TestStatusLineSubgoalCount:
    def test_status_line_no_subgoals(self, omniworker_home):
        from omniworker_cli.goals import GoalManager
    ...
    def test_status_line_with_subgoals(self, omniworker_home):
        from omniworker_cli.goals import GoalManager

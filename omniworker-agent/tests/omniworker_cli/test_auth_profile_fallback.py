...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.flux-agent/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.flux-agent            (has its own auth.json fixture)
    * Profile     -> tmp_path/.flux-agent/profiles/coder   (active, OMNIWORKER_HOME points here)
    ...
    global_root = tmp_path / ".flux-agent"
    global_root.mkdir()
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    ...
    from flux-agent_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: OMNIWORKER_HOME is set to a custom path that is NOT under
    # ~/.flux-agent/profiles/ — get_default_flux-agent_root() returns OMNIWORKER_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from flux-agent_cli.auth import read_credential_pool, write_credential_pool

...
When ``OMNIWORKER_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.omniworker/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.omniworker            (has its own auth.json fixture)
    * Profile     -> tmp_path/.omniworker/profiles/coder   (active, OMNIWORKER_HOME points here)
    ...
    global_root = tmp_path / ".omniworker"
    global_root.mkdir()
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (OMNIWORKER_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.omniworker as the "real home" — which is NOT equal
    # to the OMNIWORKER_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    omniworker_home = tmp_path / "classic"
    omniworker_home.mkdir()
    monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))

    _write(omniworker_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from omniworker_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: OMNIWORKER_HOME is set to a custom path that is NOT under
    # ~/.omniworker/profiles/ — get_default_omniworker_root() returns OMNIWORKER_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from omniworker_cli.auth import read_credential_pool, write_credential_pool

...
def fresh_home(tmp_path, monkeypatch):
    """Isolated OMNIWORKER_HOME with no prior kanban state.
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        """``OMNIWORKER_KANBAN_DB`` pins the file regardless of board= arg."""
        forced = tmp_path / "custom.db"
        monkeypatch.setenv("OMNIWORKER_KANBAN_DB", str(forced))
        assert kb.kanban_db_path() == forced
        assert kb.kanban_db_path(board="ignored") == forced

    def test_env_var_workspaces_override(self, fresh_home, tmp_path, monkeypatch):
        forced = tmp_path / "ws"
        monkeypatch.setenv("OMNIWORKER_KANBAN_WORKSPACES_ROOT", str(forced))
...
class TestCurrentBoard:
    ...
    def test_env_var_takes_precedence(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("OMNIWORKER_KANBAN_BOARD", "envboard")
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("OMNIWORKER_KANBAN_BOARD", "envwin")
        with kb.connect() as conn:
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``OMNIWORKER_KANBAN_BOARD`` / DB / workspaces on spawn.
    ...
    def test_default_spawn_sets_env_vars(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["OMNIWORKER_KANBAN_BOARD"] == "spawntest"
        assert env["OMNIWORKER_KANBAN_TASK"] == "t_abc"
        # DB path should match the per-board DB, not the legacy default.
        expected_db = fresh_home / "kanban" / "boards" / "spawntest" / "kanban.db"
        assert env["OMNIWORKER_KANBAN_DB"] == str(expected_db)
        expected_ws = fresh_home / "kanban" / "boards" / "spawntest" / "workspaces"
        assert env["OMNIWORKER_KANBAN_WORKSPACES_ROOT"] == str(expected_ws)
    ...
    def test_default_board_spawn_keeps_legacy_paths(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["OMNIWORKER_KANBAN_BOARD"] == "default"
        assert env["OMNIWORKER_KANBAN_DB"] == str(fresh_home / "kanban.db")
...
class TestCLI:
    def test_boards_list_default_only(self, tmp_path):
        env = {"OMNIWORKER_HOME": str(tmp_path)}

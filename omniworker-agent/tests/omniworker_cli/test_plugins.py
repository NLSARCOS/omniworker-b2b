"""Tests for the Flux Agent plugin system (flux-agent_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<flux-agent_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<flux-agent_home>/plugins/``; we derive
    ``<flux-agent_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <FLUX AGENT_HOME>/config.yaml.
        # Config is always read from FLUX AGENT_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        flux-agent_home_str = os.environ.get("FLUX AGENT_HOME")
        if flux-agent_home_str:
            flux-agent_home = Path(flux-agent_home_str)
        else:
            flux-agent_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.flux-agent/plugins/ are discovered."""
        plugins_dir = tmp_path / "flux-agent_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("FLUX AGENT_HOME", str(tmp_path / "flux-agent_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FLUX AGENT_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".flux-agent" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under flux-agent_plugins.<name>."""
        plugins_dir = tmp_path / "flux-agent_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("FLUX AGENT_HOME", str(tmp_path / "flux-agent_test"))

        # Clean up any prior namespace module
        sys.modules.pop("flux-agent_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "flux-agent_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="flux-agent_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "flux-agent_cli.plugins.invoke_hook",
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("flux-agent_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        flux-agent_home = tmp_path / "flux-agent_test"
        plugins_dir = flux-agent_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "flux-agent_test" / "plugins"
        # Set FLUX AGENT_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("flux-agent_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("flux-agent_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("flux-agent_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("flux-agent_cli.plugins.PluginContext.dispatch_tool.__module__", "flux-agent_cli.plugins"):
...
class TestPluginDebugLogging:
    """FLUX AGENT_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("FLUX AGENT_PLUGINS_DEBUG", raising=False)
        from flux-agent_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With FLUX AGENT_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("FLUX AGENT_PLUGINS_DEBUG", "1")
        from flux-agent_cli import plugins as plugins_mod

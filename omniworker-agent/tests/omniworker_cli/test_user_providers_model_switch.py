...
import pytest
from omniworker_cli.model_switch import list_authenticated_providers, switch_model
from omniworker_cli import runtime_provider as rp
...
def test_list_authenticated_providers_includes_full_models_list_from_user_providers(monkeypatch):
    ...
    monkeypatch.setattr("omniworker_cli.providers.OMNIWORKER_OVERLAYS", {})
...
def test_list_authenticated_providers_enumerates_dict_format_models(monkeypatch):
    ...
    (canonical OmniWorker write format) should surface every key in the picker.

    Regression: the ``providers:`` dict path previously only accepted
    list-format ``models:`` and silently dropped dict-format entries,
    even though OmniWorker's own writer and downstream readers use dict format.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("omniworker_cli.providers.OMNIWORKER_OVERLAYS", {})
...
def test_list_authenticated_providers_uses_live_models_for_user_provider(monkeypatch):
    ...
    monkeypatch.setattr("omniworker_cli.models.fetch_api_models", fake_fetch_api_models)
...
def test_list_authenticated_providers_accepts_base_url_and_singular_model(monkeypatch):
    """providers: dict entries written in canonical OmniWorker shape
    ...
    ``default_model``, so new-shape entries written by OmniWorker's own writer
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("omniworker_cli.model_switch.resolve_alias", return_value=None), \
         patch("omniworker_cli.model_switch.list_provider_models", return_value=[]), \
         patch("omniworker_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("omniworker_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         ...
         patch("omniworker_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,

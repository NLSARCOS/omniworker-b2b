"""Tests for `flux-agent curator archive` and `flux-agent curator prune`.
...
def test_archive_refuses_pinned(monkeypatch, capsys):
    ...
    assert "flux-agent curator unpin" in out
...
def test_archive_and_prune_registered():
    ...
    parser = argparse.ArgumentParser(prog="flux-agent curator")

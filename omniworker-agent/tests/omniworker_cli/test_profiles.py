"""Comprehensive tests for flux-agent_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and FLUX AGENT_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.flux-agent/profiles)
    * FLUX AGENT_HOME  -> tmp_path/.flux-agent  (so get_flux-agent_home() agrees)
    * Creates the bare-minimum ~/.flux-agent directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["flux-agent", "test", "tmp", "root", "sudo"])
...
class TestGetProfileDir:
    ...
    def test_default_returns_flux-agent_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".flux-agent" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".flux-agent" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.flux-agent must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".flux-agent"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes flux-agent-agent, .worktrees,
        ...
        (default_home / "flux-agent-agent" / ".git").mkdir(parents=True)
        (default_home / "flux-agent-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "flux-agent-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "flux-agent-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `flux-agent profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `flux-agent update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("flux-agent_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".flux-agent" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_flux-agent_home_returns_default(self, profile_env):
        # FLUX AGENT_HOME points to tmp_path/.flux-agent which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom FLUX AGENT_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("FLUX AGENT_HOME", str(custom))
        # With Docker-aware roots, a custom FLUX AGENT_HOME is the default —
...
class TestResolveProfileEnv:
    ...
    def test_existing_profile_returns_path(self, profile_env):
        ...
        assert result == str(tmp_path / ".flux-agent" / "profiles" / "coder")

    def test_default_returns_default_home(self, profile_env):
        tmp_path = profile_env
        result = resolve_profile_env("default")
        assert result == str(tmp_path / ".flux-agent")
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("flux-agent")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".flux-agent" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("flux-agent_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".flux-agent" / "profiles" / "newname"

    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        tmp_path = profile_env
        create_profile("ssi_health", no_alias=True)
        honcho_path = tmp_path / ".flux-agent" / "honcho.json"
        honcho_path.write_text(json.dumps({
            "hosts": {
                "flux-agent.ssi_health": {
                    "recallMode": "hybrid",
        ...
        assert "flux-agent.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["flux-agent.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["flux-agent.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "flux-agent.ssi_health": {"workspace": "flux-agent", "enabled": True}
            }
        }))
        ...
        assert "flux-agent.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["flux-agent.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["flux-agent.heimdall"]["workspace"] == "flux-agent"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "flux-agent.ssi_health": {"aiPeer": "ssi_health"},
                "flux-agent.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["flux-agent.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["flux-agent.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("flux-agent-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".flux-agent_history",
        ...
        excluded_prefixes = [
            "default/flux-agent-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.flux-agent_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultFlux AgentHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_flux-agent_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".flux-agent" / "profiles"

    def test_default_flux-agent_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_flux-agent_home()
        assert home == tmp_path / ".flux-agent"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (FLUX AGENT_HOME outside ~/.flux-agent), profiles go under FLUX AGENT_HOME."""
    ...
    def test_default_flux-agent_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_flux-agent_home() returns FLUX AGENT_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FLUX AGENT_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (FLUX AGENT_HOME under ~/.flux-agent), profiles root is still ~/.flux-agent/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under FLUX AGENT_HOME."""
        from flux-agent_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FLUX AGENT_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under FLUX AGENT_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FLUX AGENT_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".flux-agent" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from flux-agent_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".flux-agent"

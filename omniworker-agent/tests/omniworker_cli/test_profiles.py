"""Comprehensive tests for omniworker_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and OMNIWORKER_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.omniworker/profiles)
    * OMNIWORKER_HOME  -> tmp_path/.omniworker  (so get_omniworker_home() agrees)
    * Creates the bare-minimum ~/.omniworker directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["omniworker", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the OmniWorker install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_omniworker_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".omniworker"

    def test_named_profile_returns_profiles_subdir(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("coder")
        assert result == tmp_path / ".omniworker" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".omniworker" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.omniworker must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".omniworker"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes omniworker-agent, .worktrees,
        ...
        (default_home / "omniworker-agent" / ".git").mkdir(parents=True)
        (default_home / "omniworker-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "omniworker-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "omniworker-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `omniworker profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `omniworker update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("omniworker_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".omniworker" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_omniworker_home_returns_default(self, profile_env):
        # OMNIWORKER_HOME points to tmp_path/.omniworker which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".omniworker" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom OMNIWORKER_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("OMNIWORKER_HOME", str(custom))
        # With Docker-aware roots, a custom OMNIWORKER_HOME is the default —
...
class TestResolveProfileEnv:
    ...
    def test_existing_profile_returns_path(self, profile_env):
        ...
        assert result == str(tmp_path / ".omniworker" / "profiles" / "coder")

    def test_default_returns_default_home(self, profile_env):
        tmp_path = profile_env
        result = resolve_profile_env("default")
        assert result == str(tmp_path / ".omniworker")
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("omniworker")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".omniworker" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("omniworker_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".omniworker" / "profiles" / "newname"

    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        tmp_path = profile_env
        create_profile("ssi_health", no_alias=True)
        honcho_path = tmp_path / ".omniworker" / "honcho.json"
        honcho_path.write_text(json.dumps({
            "hosts": {
                "omniworker.ssi_health": {
                    "recallMode": "hybrid",
        ...
        assert "omniworker.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["omniworker.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["omniworker.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "omniworker.ssi_health": {"workspace": "omniworker", "enabled": True}
            }
        }))
        ...
        assert "omniworker.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["omniworker.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["omniworker.heimdall"]["workspace"] == "omniworker"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "omniworker.ssi_health": {"aiPeer": "ssi_health"},
                "omniworker.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["omniworker.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["omniworker.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("omniworker-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".omniworker_history",
        ...
        excluded_prefixes = [
            "default/omniworker-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.omniworker_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultOmniWorkerHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_omniworker_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".omniworker" / "profiles"

    def test_default_omniworker_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_omniworker_home()
        assert home == tmp_path / ".omniworker"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (OMNIWORKER_HOME outside ~/.omniworker), profiles go under OMNIWORKER_HOME."""
    ...
    def test_default_omniworker_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_omniworker_home() returns OMNIWORKER_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OMNIWORKER_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (OMNIWORKER_HOME under ~/.omniworker), profiles root is still ~/.omniworker/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under OMNIWORKER_HOME."""
        from omniworker_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OMNIWORKER_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under OMNIWORKER_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OMNIWORKER_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".omniworker" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from omniworker_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".omniworker"

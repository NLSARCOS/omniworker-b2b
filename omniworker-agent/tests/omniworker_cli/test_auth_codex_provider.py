"""Tests for Codex auth — tokens stored in Flux Agent auth store (~/.omniworker/auth.json)."""
...
def _setup_omniworker_auth(omniworker_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Flux Agent auth store."""
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Flux Agent auth store, not ~/.codex/."""
    omniworker_home = tmp_path / "omniworker"
    codex_home = tmp_path / "codex-cli"
    omniworker_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Flux Agent store
    assert not (codex_home / "auth.json").exists()

    # Flux Agent auth store should have the tokens
    data = _read_codex_tokens()

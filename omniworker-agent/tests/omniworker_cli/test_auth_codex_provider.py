"""Tests for Codex auth — tokens stored in Flux Agent auth store (~/.flux-agent/auth.json)."""
...
def _setup_flux-agent_auth(flux-agent_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    ...
    auth_file = flux-agent_home / "auth.json"
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_flux-agent_auth(flux-agent_home)
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    flux-agent_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (flux-agent_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("FLUX AGENT_HOME", str(flux-agent_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    flux-agent_home = tmp_path / "flux-agent"
    _setup_flux-agent_auth(flux-agent_home, access_token="")
    monkeypatch.setenv("FLUX AGENT_HOME", str(flux-agent_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_flux-agent_auth(flux-agent_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("FLUX AGENT_HOME", str(flux-agent_home))
    ...
    monkeypatch.setattr("flux-agent_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    flux-agent_home = tmp_path / "flux-agent"
    _setup_flux-agent_auth(flux-agent_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("FLUX AGENT_HOME", str(flux-agent_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    _save_codex_tokens({"access_token": "flux-agent-at", "refresh_token": "flux-agent-rt"})
    ...
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "flux-agent-at"


def test_resolve_returns_flux-agent_auth_store_source(tmp_path, monkeypatch):
    flux-agent_home = tmp_path / "flux-agent"
    _setup_flux-agent_auth(flux-agent_home)
    monkeypatch.setenv("FLUX AGENT_HOME", str(flux-agent_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "flux-agent-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("flux-agent_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "flux-agent_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("flux-agent_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("flux-agent_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",

"""Tests for the `omniworker memory reset` CLI command.
...
- Profile-scoped reset (uses OMNIWORKER_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake OMNIWORKER_HOME with memory files."""
    omniworker_home = tmp_path / ".omniworker"
    memories = omniworker_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nOmniWorker repo is at ~/.omniworker/omniworker-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
    ...
    return omniworker_home, memories
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `omniworker memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        omniworker_home = tmp_path / ".omniworker"
        (omniworker_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's OMNIWORKER_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        omniworker_home = tmp_path / ".omniworker"
        omniworker_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("OMNIWORKER_HOME", str(omniworker_home))

        # The memories dir won't exist; get_omniworker_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"

"""Tests for the `flux-agent memory reset` CLI command.
...
- Profile-scoped reset (uses FLUX AGENT_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake FLUX AGENT_HOME with memory files."""
    flux-agent_home = tmp_path / ".flux-agent"
    memories = flux-agent_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("FLUX AGENT_HOME", str(flux-agent_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nFlux Agent repo is at ~/.flux-agent/flux-agent-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
    ...
    return flux-agent_home, memories
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `flux-agent memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        flux-agent_home = tmp_path / ".flux-agent"
        (flux-agent_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("FLUX AGENT_HOME", str(flux-agent_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's FLUX AGENT_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        flux-agent_home = tmp_path / ".flux-agent"
        flux-agent_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("FLUX AGENT_HOME", str(flux-agent_home))

        # The memories dir won't exist; get_flux-agent_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"

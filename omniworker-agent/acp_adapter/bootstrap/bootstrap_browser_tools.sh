...
# into ~/.omniworker/node/ for use by OmniWorker Agent's browser tools.
#
# Targets the registry-install path: users who got OmniWorker via
# `uvx --from 'omniworker-agent[acp]==X' omniworker-acp` don't have a repo clone,
...
# safe to run from `omniworker-acp --setup-browser`, from a fresh terminal,
...
#   OMNIWORKER_HOME=/custom/path bootstrap_browser_tools.sh
#
...
NODE_PREFIX="$OMNIWORKER_HOME/node"
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            cat <<EOF
Bootstrap OmniWorker Agent browser tools.

Installs Node.js (into ~/.omniworker/node/), the agent-browser npm package,
...
Environment:
  OMNIWORKER_HOME       Override OmniWorker data dir (default: \$HOME/.omniworker)
EOF
            exit 0
            ;;
        *)
...
ensure_node() {
    ...
    if [ -x "$NODE_PREFIX/bin/node" ]; then
        ...
        log_success "Node.js $found_ver found (OmniWorker-managed at $NODE_PREFIX)"
        return 0
    fi
...
ensure_agent_browser() {
    ...
    # _find_agent_browser() in tools/browser_tool.py walks ~/.omniworker/node/bin
    ...
    # sudo. Force the prefix to the user-writable OmniWorker-managed Node
...
main() {
    log_info "OmniWorker Agent: bootstrapping browser tools"
    log_info "  OMNIWORKER_HOME = $OMNIWORKER_HOME"
    ...
    log_info "OmniWorker Agent will pick up agent-browser from $NODE_PREFIX/bin/ on next launch."
}

...
# into ~/.omniworker/node/ for use by OmniWorker Agent's browser tools on Windows.
#
...
$OmniWorkerHome = $env:OMNIWORKER_HOME
if (-not $OmniWorkerHome) {
    $OmniWorkerHome = Join-Path $env:USERPROFILE ".omniworker"
}
$NodePrefix = Join-Path $OmniWorkerHome "node"
...
function Ensure-Node {
    ...
    # OmniWorker-managed Node?
    $managedNode = Join-Path $NodePrefix "node.exe"
    if (Test-Path $managedNode) {
        $v = & $managedNode --version
        Write-Success "Node.js $v found (OmniWorker-managed at $NodePrefix)"
    ...
    try {
        ...
        $tmpDir = Join-Path $env:TEMP "omniworker-node-$([guid]::NewGuid().ToString('N'))"
        ...
        New-Item -ItemType Directory -Force -Path $OmniWorkerHome | Out-Null
...
function Ensure-AgentBrowser {
    ...
    # prefix to the user-writable OmniWorker-managed Node directory so we never
...
function Write-BrowserEnv {
    ...
    $envFile = Join-Path $OmniWorkerHome ".env"
    New-Item -ItemType Directory -Force -Path $OmniWorkerHome | Out-Null
    ...
    Add-Content -Path $envFile -Value "# OmniWorker Agent browser tools — use the system Chrome/Chromium/Edge binary."
...
Write-Info "OmniWorker Agent: bootstrapping browser tools"
Write-Info "  OMNIWORKER_HOME = $OmniWorkerHome"
...
Write-Info "OmniWorker Agent will pick up agent-browser from $NodePrefix on next launch."

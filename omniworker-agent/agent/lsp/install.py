...
appropriate.  All installs go to a Flux Agent-owned bin staging dir,
``<FLUX AGENT_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``flux-agent lsp
  status``.
...
# ``<FLUX AGENT_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def flux-agent_lsp_bin_dir() -> Path:
    """Return the Flux Agent-owned bin staging dir for LSP servers."""
    home = os.environ.get("FLUX AGENT_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".flux-agent")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    staged = flux-agent_lsp_bin_dir() / name
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = flux-agent_lsp_bin_dir().parent  # <FLUX AGENT_HOME>/lsp/
    ...
    for c in candidates:
        if c.exists():
            ...
            link = flux-agent_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_go(pkg: str, bin_name: str) -> Optional[str]:
    ...
    if go is None:
        ...
        return None
    staging = flux-agent_lsp_bin_dir()
    env = dict(os.environ)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a flux-agent-owned target dir.
    ...
    console script.
    """
    pip_target = flux-agent_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = flux-agent_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``flux-agent lsp status`` CLI to give users a quick

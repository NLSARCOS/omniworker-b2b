...
appropriate.  All installs go to a OmniWorker-owned bin staging dir,
``<OMNIWORKER_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``omniworker lsp
  status``.
...
# ``<OMNIWORKER_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def omniworker_lsp_bin_dir() -> Path:
    """Return the OmniWorker-owned bin staging dir for LSP servers."""
    home = os.environ.get("OMNIWORKER_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".omniworker")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    staged = omniworker_lsp_bin_dir() / name
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = omniworker_lsp_bin_dir().parent  # <OMNIWORKER_HOME>/lsp/
    ...
    for c in candidates:
        if c.exists():
            ...
            link = omniworker_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_go(pkg: str, bin_name: str) -> Optional[str]:
    ...
    if go is None:
        ...
        return None
    staging = omniworker_lsp_bin_dir()
    env = dict(os.environ)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a omniworker-owned target dir.
    ...
    console script.
    """
    pip_target = omniworker_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = omniworker_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``omniworker lsp status`` CLI to give users a quick

...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``OMNIWORKER_PLATFORM`` env var and
    ``OMNIWORKER_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("OMNIWORKER_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_omniworker_home()}/config.yaml):"]

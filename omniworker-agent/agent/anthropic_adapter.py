"""Anthropic Messages API adapter for OmniWorker Agent.

Translates between OmniWorker's internal OpenAI-style message format and
...
# OmniWorker effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _resolve_positive_anthropic_max_tokens(value) -> Optional[int]:
    ...
    finite positive number. Ported from omniworker/omniworker#66664.
...
def _resolve_anthropic_messages_max_tokens(
    ...
    Ported from omniworker/omniworker#66664 (resolveAnthropicMessagesMaxTokens).
    """
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See omniworker-agent#13848, #17057.
    """
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    OmniWorker historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by OmniWorker)
    ...
    # 1. OmniWorker-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration OmniWorker configs.
...
# ── OmniWorker-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.omniworker/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_OMNIWORKER_OAUTH_FILE = get_omniworker_home() / ".anthropic_oauth.json"
...
def run_omniworker_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run OmniWorker-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize OmniWorker with your Claude Pro/Max subscription.")
    print()
...
def read_omniworker_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read OmniWorker-managed OAuth credentials from ~/.omniworker/.anthropic_oauth.json."""
    if _OMNIWORKER_OAUTH_FILE.exists():
        try:
            data = json.loads(_OMNIWORKER_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read OmniWorker OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See omniworker-agent#13848.
            #
    ...
    #
    # For direct Anthropic (strategy following clawdbot/OmniWorker):
    ...
    # are preserved.  See omniworker-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    # rather than 400-ing at the Anthropic API. See omniworker/omniworker#66664.
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("OmniWorker Agent", "Claude Code")
                text = text.replace("OmniWorker agent", "Claude Code")
                text = text.replace("omniworker-agent", "claude-code")
    ...
    # silently hides reasoning text that OmniWorker surfaces in its CLI. We

...
Plugins built on Flux Agent Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Flux Agent
...
embedded slugs, no shorthands. This mirrors Flux Agent' main config
...
handles every provider, fallback chain, and per-task override Flux Agent
supports.
"""

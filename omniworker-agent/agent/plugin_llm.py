...
Plugins built on OmniWorker Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing OmniWorker
...
:class:`~omniworker_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors OmniWorker' main config
...
handles every provider, fallback chain, and per-task override OmniWorker
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`omniworker_cli.plugins.PluginContext`

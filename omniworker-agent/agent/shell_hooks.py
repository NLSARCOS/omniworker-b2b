...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # OmniWorker-canonical
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    "reason": "..."}`` payload is translated into the canonical OmniWorker
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ OmniWorker is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    holding the canonical OmniWorker-wire-shape response."""

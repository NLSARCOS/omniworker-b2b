...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Flux Agent-canonical
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Flux Agent wire-shape dict.
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Flux Agent is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    holding the canonical Flux Agent-wire-shape response."""

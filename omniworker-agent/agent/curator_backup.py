...
A pre-run snapshot of ``~/.flux-agent/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.flux-agent/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.flux-agent/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_flux-agent_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.flux-agent/cron/jobs.json``)."""
    return get_flux-agent_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.flux-agent/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.flux-agent/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.flux-agent/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.flux-agent/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `flux-agent curator rollback --list` to see available snapshots)",
            None,
        )

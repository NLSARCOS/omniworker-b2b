...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.omniworker/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``omniworker update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``omniworker curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `omniworker curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_DRY_RUN_BANNER = (
    ...
    "`omniworker curator run` (no flag).\n"
    "\n"
...
CURATOR_REVIEW_PROMPT = (
    "You are running as OmniWorker' background skill CURATOR. This is an "
    ...
    "into ~/.omniworker/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: omniworker-config-*, omniworker-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.omniworker/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.omniworker/logs/curator/``)
    ...
    authored skill data in ``~/.omniworker/skills/``.
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every omniworker install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        keep an umbrella stable: omniworker curator pin document-tools
    ...
    lines.append("full report: omniworker curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: omniworker curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.omniworker/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.omniworker/skills/.archive/` for "
            "safety and can be restored via `omniworker curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.omniworker/skills/.archive/`. "
            "Restore any via `omniworker curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `omniworker curator restore <name>`")
    lines.append("- All archives live under `~/.omniworker/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `omniworker curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `omniworker curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``omniworker model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `omniworker model` → auxiliary

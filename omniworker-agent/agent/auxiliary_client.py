...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.omniworker/auth.json active provider)
...
_NOUS_DEFAULT_BASE_URL = "https://inference-api.omniworker.com/v1"
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.omniworker/auth.json for an active Nous provider.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `omniworker setup` or `omniworker model` and enter a valid http(s) base URL."
        ) from exc
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second omniworker
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``omniworker config aux reset``)."""
...
def _recoverable_pool_provider(resolved_provider: str, client: Any) -> Optional[str]:
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.omniworker.com"):
        return "nous"
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `omniworker model` but the
    #    old OPENAI_BASE_URL lingers in ~/.omniworker/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: omniworker model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.omniworker/.env",
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: omniworker auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: omniworker model)")
                return None, None
    ...
    *,
    ...
    try:
        ...
        client_is_nous = (
            ...
            or base_url_host_matches(_base_info, "inference-api.omniworker.com")
        )
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.omniworker.com")
        )
        if _is_auth_error(first_err) and client_is_nous:
            refreshed_client, refreshed_model = _refresh_nous_auxiliary_client(
                cache_provider=resolved_provider or "nous",

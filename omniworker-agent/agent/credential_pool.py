...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``omniworker model`` / ``omniworker auth`` which
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("omniworker_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("omniworker_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # re-authed via `omniworker model` / `omniworker auth` after a 429/401,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `omniworker auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_omniworker_oauth_credentials

        for source_name, creds in (
            ("omniworker_pkce", read_omniworker_oauth_credentials()),
        ...
        if state and not _is_suppressed(provider, "device_code"):
            ...
            # `omniworker auth add nous --label <name>`).  Fall back to the
        ...
        # MiniMax OAuth tokens live in ~/.omniworker/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `omniworker auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `omniworker auth remove openai-codex` marks
        ...
        # via `omniworker auth openai-codex`.
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.omniworker/.env over os.environ — the user's config file is the
    ...
    # Honour user suppression — `omniworker auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.omniworker/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.omniworker/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "omniworker_pkce"}
        )
    ]

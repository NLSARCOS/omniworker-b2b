"""Projects codex app-server events into Flux Agent' messages list.

The translator that lets Flux Agent' memory/skill review keep working under the
...
Flux Agent' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Flux Agent
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Flux Agent' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Flux Agent stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Flux Agent already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Flux Agent' messages store text only.
        text_parts: list[str] = []

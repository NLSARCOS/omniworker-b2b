...
MEMORY_GUIDANCE = (
    "You have a powerful persistent memory system that works across sessions:\n"
    "1. **NativeMemory (automatic):** Every conversation turn is indexed with full-text "
    "search (FTS5) + semantic vector search. Relevant past context is automatically "
    "recalled and injected when you need it — no action required from you. Memory "
    "recall spans 30 days and supports both keyword AND semantic matching.\n"
    "2. **Explicit memory tool:** For facts you want to guarantee are always visible, "
    "use the memory tool to save durable facts: user preferences, environment details, "
    "tool quirks, and stable conventions.\n"
    "3. **session_search:** For recalling specific past conversations or task outcomes.\n\n"
    "Prioritize what reduces future user steering — the most valuable memory is one "
    "that prevents the user from having to correct or remind you again. "
    "User preferences and recurring corrections matter more than procedural task details.\n"
    "Do NOT save task progress, session outcomes, completed-work logs, or temporary TODO "
    "state to the memory tool; use session_search to recall those from past transcripts. "
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "and follow its instructions. For Flux Agent config/setup always load `omniworker-agent` first.\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>\n"

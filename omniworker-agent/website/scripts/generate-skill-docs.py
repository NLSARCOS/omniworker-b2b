...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Flux Agent/omniworker-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Flux Agent ships with a large built-in skill library copied into `~/.omniworker/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Flux Agent also syncs bundled skills on `omniworker update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.omniworker/skills/` tree, it is still shipped with Flux Agent; restore it with `omniworker skills reset <name> --restore`.",
        "",

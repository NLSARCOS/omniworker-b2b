...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Flux Agent/flux-agent-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    flux-agent_meta = (fm.get("metadata") or {}).get("flux-agent") or {}
    tags = flux-agent_meta.get("tags") or []
    related = flux-agent_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `flux-agent skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Flux Agent Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Flux Agent ships with a large built-in skill library copied into `~/.flux-agent/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Flux Agent also syncs bundled skills on `flux-agent update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.flux-agent/skills/` tree, it is still shipped with Flux Agent; restore it with `flux-agent skills reset <name> --restore`.",
        "",

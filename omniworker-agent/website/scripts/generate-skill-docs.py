...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/OmniWorker/omniworker-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    omniworker_meta = (fm.get("metadata") or {}).get("omniworker") or {}
    tags = omniworker_meta.get("tags") or []
    related = omniworker_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `omniworker skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with OmniWorker Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "OmniWorker ships with a large built-in skill library copied into `~/.omniworker/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "OmniWorker also syncs bundled skills on `omniworker update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.omniworker/skills/` tree, it is still shipped with OmniWorker; restore it with `omniworker skills reset <name> --restore`.",
        "",

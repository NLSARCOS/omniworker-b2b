...
description: "Authoritative reference for Flux Agent terminal commands and command families"
---
...
The report includes system info (OS, Python version, Flux Agent version), recent agent and gateway logs (512 KB limit per file), and redacted API key status. Keys are always redacted — no secrets are uploaded.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Flux Agent is running (WAL-mode safe).
...
| Subcommand | Description |
|------------|-------------|
| `serve [-v\|--verbose]` | Run Flux Agent as an MCP server — expose conversations to other agents. |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Flux Agent equivalents or **archived** for manual review.

...
flux-agent skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.flux-agent/skills/`
...
```bash
# Check it's there
flux-agent skills list | grep arxiv
...
For how to ship skills in your own plugin, see [Build a Flux Agent Plugin → Bundle skills](/docs/guides/build-a-flux-agent-plugin#bundle-skills).
...
flux-agent skills config gif-search

# View all skill config
flux-agent config get skills.config
```
...
```bash
mkdir -p ~/.flux-agent/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.flux-agent/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
flux-agent chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.flux-agent/skills/` and it's live.
...
**Use categories.** Organize skills into subdirectories (`~/.flux-agent/skills/devops/`, `~/.flux-agent/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

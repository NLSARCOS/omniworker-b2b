...
FLUX AGENT_STREAM_READ_TIMEOUT=1800
```

| Timeout | Default | Local auto-adjustment | Env var override |
|---------|---------|----------------------|------------------|
| Stream read (socket-level) | 120s | Raised to 1800s | `FLUX AGENT_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Disabled entirely | `FLUX AGENT_STREAM_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | No change needed | `FLUX AGENT_API_TIMEOUT` |

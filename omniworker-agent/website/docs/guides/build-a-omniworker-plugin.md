...
Flux Agent has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.omniworker/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
Model Context Protocol (MCP) servers register their own tools into Flux Agent without any Python plugin. Declare them in `~/.omniworker/config.yaml`:

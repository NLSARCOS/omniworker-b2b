...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  OmniWorker Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to OmniWorker instantly when PRs are opened or updated.
:::
...
- **OmniWorker Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
Make sure OmniWorker can access GitHub. Start a chat:
...
```
Run: gh pr list --repo OmniWorker/omniworker-agent --state open --limit 3
```
...
Still in the chat, ask OmniWorker to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo OmniWorker/omniworker-agent
```
...
A skill gives OmniWorker consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.
...
This is what makes the reviewer actually useful. Start a session and teach OmniWorker your team's standards:
...
2. Teach OmniWorker your conventions via memory (Step 4)

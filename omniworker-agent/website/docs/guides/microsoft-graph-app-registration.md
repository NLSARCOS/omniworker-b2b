...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.omniworker/.env` at the end.
...
4. Fill in:
   - **Name:** `OmniWorker Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `omniworker-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the OmniWorker app
New-CsApplicationAccessPolicy `
  -Identity "OmniWorker-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict OmniWorker meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "OmniWorker-Meeting-Pipeline-Policy" `
...
Put the three values you collected into `~/.omniworker/.env`:
...
OmniWorker ships a Graph auth smoke-test. From your OmniWorker install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.omniworker/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `omniworker gateway restart`.

...
4. Fill in:
   - **Name:** `Flux Agent Teams Meeting Pipeline` (or any name you'll recognize).
...
```powershell
# Create a policy scoped to the Flux Agent app
New-CsApplicationAccessPolicy `
  -Identity "Flux Agent-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Flux Agent meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Flux Agent-Meeting-Pipeline-Policy" `
...
Flux Agent ships a Graph auth smoke-test. From your Flux Agent install:

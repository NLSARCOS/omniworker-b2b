...
description: "Diagnose and fix common OmniWorker cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
The `script` path must be an absolute path (or relative to the OmniWorker config directory). Verify:
```bash
...
omniworker logs                         # View recent OmniWorker logs
...
3. Open an issue at [github.com/OmniWorker/omniworker-agent](https://github.com/OmniWorker/omniworker-agent) with:

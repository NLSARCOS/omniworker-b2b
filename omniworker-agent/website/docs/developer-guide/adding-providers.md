...
description: "How to add a new inference provider to Flux Agent Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Flux Agent can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:
...
- What env vars should Flux Agent check, and in what priority order?
...
When you add a native provider, double-check that Flux Agent is only sending fields that provider actually understands.

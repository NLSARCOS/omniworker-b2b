...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept OmniWorker already has applies. The plugin never sees a
...
[`omniworker-example-plugins`](https://github.com/OmniWorker/omniworker-example-plugins/tree/main/plugin-llm-example)
...
[`plugin-llm-async-example`](https://github.com/OmniWorker/omniworker-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — OmniWorker provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different OmniWorker provider
        # (must be one OmniWorker already knows about — same names as
        ...
        # plugin sends — OmniWorker does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through OmniWorker' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing OmniWorker subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/OmniWorker/omniworker-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/OmniWorker/omniworker-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/OmniWorker/omniworker-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/OmniWorker/omniworker-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`

...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Flux Agent already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Flux Agent provider id (e.g. "openrouter")
  ...
  entries:
    my-plugin:
      llm:
        ...
        # plugin sends — Flux Agent does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Flux Agent' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Flux Agent subsystem:

...
[PR #12550](https://github.com/OmniWorker/omniworker-agent/pull/12550) proposed a
...
(`omniworker-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per OmniWorker `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `omniworker-acp`, `omniworker-api-server`, core toolsets (gated on CDP reachability)
...
- `omniworker_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`

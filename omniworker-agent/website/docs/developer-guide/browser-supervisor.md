...
[PR #12550](https://github.com/Flux Agent/flux-agent-agent/pull/12550) proposed a
...
(`flux-agent-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Flux Agent `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `flux-agent-acp`, `flux-agent-api-server`, core toolsets (gated on CDP reachability)
...
- `flux-agent_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`

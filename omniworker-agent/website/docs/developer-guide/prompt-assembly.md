...
description: "How Flux Agent builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Flux Agent deliberately separates:
...
You are Flux Agent, an AI assistant created by Nous Research.
...
|----------|-------|-------------|-------|
| 1 | `.omniworker.md`, `OMNIWORKER.md` | CWD up to git root | Flux Agent-native project config |
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Flux Agent already loads, rather than editing Python templates in place.
...
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Flux Agent.
...
- if you want to change how Flux Agent assembles prompts for everyone, change Python and treat it as a code contribution

...
1. agent identity — `SOUL.md` from `FLUX AGENT_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.flux-agent/SOUL.md)
...
`SOUL.md` lives at `~/.flux-agent/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_flux-agent_md(cwd_path)       # 1. .flux-agent.md / FLUX AGENT.md (walks to git root)
    ...
    # SOUL.md from FLUX AGENT_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.flux-agent.md`, `FLUX AGENT.md` | CWD up to git root | Flux Agent-native project config |
...
- **YAML frontmatter stripped** — `.flux-agent.md` frontmatter is removed (reserved for future config overrides)
...
1. `.flux-agent.md` / `FLUX AGENT.md` (walks to git root)
...
- `~/.flux-agent/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.flux-agent/MEMORY.md` and `~/.flux-agent/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.flux-agent.md`, `FLUX AGENT.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
...
- Ephemeral overlays such as `FLUX AGENT_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.

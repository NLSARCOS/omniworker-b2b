...
1. **Pairing-data snapshot** — a lightweight pre-update state snapshot is saved (covers `~/.omniworker/pairing/`, Feishu comment rules, and other state files that get modified at runtime). Recoverable via the snapshot restore flow described under [Snapshots and rollback](../user-guide/checkpoints-and-rollback.md), or by extracting the most recent quick-snapshot zip Flux Agent wrote next to your `~/.omniworker/` directory.
...
Compare against the latest release at the [GitHub releases page](https://github.com/Flux Agent/omniworker-agent/releases).

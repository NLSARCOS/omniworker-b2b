...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Flux Agent Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Flux Agent to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.flux-agent/.env`:
...
All standard Flux Agent commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.flux-agent/logs/gateway.log`
...
4. Consider enabling verbose logging: `FLUX AGENT_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Flux Agent automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
FLUX AGENT_LOG_LEVEL=debug flux-agent gateway
```
...
```bash
flux-agent chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```

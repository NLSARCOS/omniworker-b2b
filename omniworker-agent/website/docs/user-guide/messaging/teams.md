...
```bash
# devtunnel (Microsoft)
devtunnel create flux-agent-bot --allow-anonymous
devtunnel port create flux-agent-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host flux-agent-bot
...
```bash
FLUX AGENT_UID=$(id -u) FLUX AGENT_GID=$(id -g) docker compose up -d gateway
```
...
Alternatively, configure via `~/.flux-agent/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.flux-agent/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create flux-agent-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs flux-agent` for tracebacks |
...
- Store credentials in `~/.flux-agent/.env` with permissions `600` (`chmod 600 ~/.flux-agent/.env`)

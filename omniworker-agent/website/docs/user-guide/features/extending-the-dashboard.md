...
description: "Build themes and plugins for the Flux Agent web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Flux Agent web dashboard (`omniworker dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
|-------|---------|------------|--------|
| **Flux Agent Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Flux Agent Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Flux Agent Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Flux Agent plugins in `~/.omniworker/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.
...
// Flux Agent API client + raw fetcher
...
#### Calling built-in Flux Agent endpoints
...
| `header-left` | Before the Flux Agent brand in the top bar. |
...
A reference plugin (`example-dashboard` in [`omniworker-example-plugins`](https://github.com/Flux Agent/omniworker-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
#### Accessing Flux Agent internals
...
The [`strike-freedom-cockpit`](https://github.com/Flux Agent/omniworker-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `omniworker-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/Flux Agent/omniworker-example-plugins.git
...
Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Flux Agent Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).

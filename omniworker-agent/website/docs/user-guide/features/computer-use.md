...
OmniWorker Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
window-focus bug for UTM workflows). OmniWorker refreshes the binary in two
places so you don't get stuck on a stale release:

- **`omniworker update`** — when you update OmniWorker itself, if `cua-driver` is
...
OmniWorker applies multi-layer guardrails:
...
Screenshots are expensive. OmniWorker applies four layers of optimisation:
...
- [Universal skill: `macos-computer-use`](https://github.com/OmniWorker/omniworker-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)

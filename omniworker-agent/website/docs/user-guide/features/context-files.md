...
Flux Agent Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Flux Agent instance and is loaded from `OMNIWORKER_HOME` only.
...
| **SOUL.md** | Global personality and tone customization for this Flux Agent instance | `OMNIWORKER_HOME/SOUL.md` only |
...
- `~/.omniworker/SOUL.md`
- or `$OMNIWORKER_HOME/SOUL.md` if you run Flux Agent with a custom home directory

Important details:

- Flux Agent seeds a default `SOUL.md` automatically if one does not exist yet
- Flux Agent loads `SOUL.md` only from `OMNIWORKER_HOME`
- Flux Agent does not probe the working directory for `SOUL.md`
...
Flux Agent is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.omniworker.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.

This means your existing Cursor conventions automatically apply when using Flux Agent.

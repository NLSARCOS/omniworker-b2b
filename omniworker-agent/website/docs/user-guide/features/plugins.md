...
title: "Plugins"
description: "Extend Flux Agent with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Flux Agent has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
Start Flux Agent — your tools appear alongside built-in tools. The model can call them immediately.
...
Within each source, Flux Agent also recognizes sub-category directories that route plugins to specialized discovery systems:
...
Several categories of plugin bypass `plugins.enabled` — they're part of Flux Agent' built-in surface and would break basic functionality if gated off by default:
...
Flux Agent has four kinds of plugins:
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Flux Agent also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

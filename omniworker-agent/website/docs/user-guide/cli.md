...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Flux Agent Agent terminal interface — commands, keybindings, personalities, and more"
---
...
<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Flux Agent CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Flux Agent CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Flux Agent to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Flux Agent supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Flux Agent replies |
...
personalities:
  ...
  pirate: "Arrr! Ye be talkin' to Captain Flux Agent..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Flux Agent recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Flux Agent — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Flux Agent is working, Flux Agent prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Flux Agent to the background — just like any terminal process. The shell prints a confirmation:

```
Flux Agent Agent has been suspended. Run `fg` to bring Flux Agent Agent back.
```
...
Flux Agent immediately confirms the task and gives you back the prompt:

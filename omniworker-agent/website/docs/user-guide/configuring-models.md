...
Flux Agent uses two kinds of model slots:
...
- **Right** — the curated model list for the selected provider. These are the agentic models Flux Agent recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).
...
Every auxiliary task defaults to `auto` — meaning Flux Agent uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
`provider: auto` with `model: ''` tells Flux Agent to use the main model for that task.
...
### I picked a model but Flux Agent switched providers on me
...
curl -H "X-Flux Agent-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-Flux Agent-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-Flux Agent-Session-Token: $TOKEN" \

...
Flux Agent Agent has a built-in MCP client that connects to MCP servers at startup, discovers their tools, and makes them available as first-class tools the agent can call directly. No bridge CLI needed -- tools from MCP servers appear alongside built-in tools like `terminal`, `read_file`, etc.

## When to Use

Use this whenever you want to:
- Connect to MCP servers and use their tools from within Flux Agent Agent
...
Restart Flux Agent Agent. On startup it will:
...
The most common transport. Flux Agent launches the MCP server as a subprocess and communicates over stdin/stdout.
...
For stdio servers, Flux Agent does NOT pass your full shell environment to MCP subprocesses. Only safe baseline variables are inherited:
...
- Look at Flux Agent Agent startup logs for connection messages
...
Flux Agent supports MCP's `sampling/createMessage` capability — MCP servers can request LLM completions through the agent during tool execution. This enables agent-in-the-loop workflows (data analysis, content generation, decision-making).

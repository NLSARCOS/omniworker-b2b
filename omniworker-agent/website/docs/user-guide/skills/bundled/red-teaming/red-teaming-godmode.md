...
5. **Flux Agent models don't need jailbreaking** — omniworker/omniworker-3-* and omniworker-4-* are already uncensored. Use them directly for the fastest path.
...
12. **execute_code sandbox has no env vars** — When Flux Agent runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.omniworker/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.omniworker/.env"))`

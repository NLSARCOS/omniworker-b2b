...
description: "Operate the Teams meeting summary pipeline via OmniWorker CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Operate the Teams meeting summary pipeline via OmniWorker CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `omniworker teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `~/.omniworker/.env`:
...
```bash
omniworker teams-pipeline validate              # config snapshot — run first after any change
omniworker teams-pipeline token-health          # Graph token status
omniworker teams-pipeline token-health --force-refresh   # force a fresh token acquisition
omniworker teams-pipeline list                  # recent meeting jobs
omniworker teams-pipeline list --status failed  # only failed jobs
omniworker teams-pipeline show <job-id>         # full detail of one job
omniworker teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
omniworker teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
omniworker teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
omniworker teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```
...
omniworker teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
omniworker teams-pipeline delete-subscription <sub-id>
omniworker teams-pipeline maintain-subscriptions            # renew near-expiry ones
omniworker teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `omniworker teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `omniworker cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).

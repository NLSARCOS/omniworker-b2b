...
| | |
|---|---|
| Source | Optional — install with `flux-agent skills install official/productivity/telephony` |
...
- save provider credentials into `~/.flux-agent/.env`
...
### `~/.flux-agent/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.flux-agent/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
flux-agent skills search telephony
flux-agent skills install official/productivity/telephony
```
...
3. Buy it and save it into `~/.flux-agent/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.flux-agent/.env`

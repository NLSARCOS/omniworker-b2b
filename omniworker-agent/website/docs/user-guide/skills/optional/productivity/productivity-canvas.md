...
| | |
|---|---|
| Source | Optional — install with `omniworker skills install official/productivity/canvas` |
...
4. Name the token (e.g., "OmniWorker Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.omniworker/.env`:
...
```bash
CANVAS="python $OMNIWORKER_HOME/skills/productivity/canvas/scripts/canvas_api.py"

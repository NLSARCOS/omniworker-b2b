...
| | |
|---|---|
| Source | Optional — install with `flux-agent skills install official/productivity/canvas` |
...
5. Copy the token and add to `~/.flux-agent/.env`:
...
```bash
CANVAS="python $FLUX AGENT_HOME/skills/productivity/canvas/scripts/canvas_api.py"

...
The PID file lives at ``{FLUX AGENT_HOME}/gateway.pid``.  FLUX AGENT_HOME defaults to
``~/.hermes`` but can be overridden via the environment variable.  This means
separate FLUX AGENT_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting FLUX AGENT_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("FLUX AGENT_GATEWAY_LOCK_DIR")
    if override:
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Flux Agent gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different FLUX AGENT_HOME dirs).
    """

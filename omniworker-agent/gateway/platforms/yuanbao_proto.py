    ...
    └── ConnMsg (protobuf: trpc.yuanbao.conn_common.ConnMsg)
          ...
          └── data: bytes  (业务 payload，标准 protobuf)
                └── InboundMessagePush / SendC2CMessageReq / SendGroupMessageReq / ...
                      (trpc.yuanbao.yuanbao_conn.yuanbao_flux-agent_proxy.*)
...
# biz 层服务/方法映射
# TS client uses the short name 'yuanbao_flux-agent_proxy' (not the full package path)
_BIZ_PKG = "yuanbao_flux-agent_proxy"
BIZ_SERVICES = {
...
def encode_biz_msg(service: str, method: str, req_id: str, body: bytes) -> bytes:
    ...
    Args:
        service: 模块名（head.module），如 "yuanbao_flux-agent_proxy"
...
def encode_auth_bind(
    ...
    dev_buf += _encode_field(10, WT_LEN, _encode_string(str(OMNIWORKER_INSTANCE_ID)))
    if bot_version:

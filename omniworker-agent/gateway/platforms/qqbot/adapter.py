...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.flux-agent/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. Flux Agent upgrades overwrite this
    ...
    async def _listen_loop(self) -> None:
        ...
        Close code handling follows the Flux Agent qqbot reference implementation:
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "flux-agent-agent",
                    "$device": "flux-agent-agent",
                },
            },
        }
        try:
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.flux-agent/.update_response`` for the
          detached ``flux-agent update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``flux-agent update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    async def _process_attachments(
        ...
        Mirrors Flux Agent's ``processAttachments`` — handles images, voice, and
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 1. Plugin-specific STT config (matches Flux Agent's channels.qqbot.stt)
        ...
        # 2. QQ-specific env vars (set by `flux-agent setup gateway` / `flux-agent gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``flux-agent update --gateway`` watcher. Button
        ...
        ``~/.flux-agent/.update_response`` so the detached update process
        can read it.
        """

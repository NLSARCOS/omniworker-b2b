...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Flux Agent-Session-Id header; opt-in long-term memory scoping via X-Flux Agent-Session-Key header)
...
def _derive_chat_session_id(
    ...
    the same Flux Agent session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Flux Agent-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Flux Agent-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Flux Agent-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Flux Agent-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Flux Agent-Session-Key requires API key authentication. "
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Flux Agent version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Flux Agent AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Flux Agent-Session-Id",
                "session_key_header": "X-Flux Agent-Session-Key",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Flux Agent-Session-Key.  This
        # is independent of X-Flux Agent-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Flux Agent-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Flux Agent-Session-Id", "").strip()
        if provided_session_id:
            # Allow localhost clients (desktop, CLI) to continue sessions
            # without API_SERVER_KEY.  Remote/gateway clients still require
            # authentication to prevent session ID enumeration attacks.
            _peer = request.remote or ""
            _is_localhost = _peer in ("127.0.0.1", "::1", "localhost")
            if not self._api_key and not _is_localhost:
                logger.warning(
                    "Session continuation via X-Flux Agent-Session-Id rejected: "
            ...
            # conversation map to the same Flux Agent session.  The first user
        ...
        response_headers = {
            "X-Flux Agent-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Flux Agent-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-Flux Agent-Completed"] = "false"
            response_headers["X-Flux Agent-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Flux Agent-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Flux Agent-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Flux Agent-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Flux Agent-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Flux Agent-Session-Key"] = gateway_session_key
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Flux Agent-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Flux Agent-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Flux Agent-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_get_session_workspace(self, request: "web.Request") -> "web.Response":
        """GET /v1/sessions/{session_id}/workspace — retrieve structured workspace state.

        Returns the active workspace state for a session: files edited, pending
        errors, active goals, recent decisions, and last task summary.  Used by
        the SaaS Context Bridge to inject structured continuity context across
        model switches instead of a lossy narrative summary.
        """
        # Allow localhost without API key (same rule as session continuation)
        _peer = request.remote or ""
        _is_localhost = _peer in ("127.0.0.1", "::1", "localhost")
        if not self._api_key and not _is_localhost:
            return web.json_response(
                _openai_error(
                    "Session workspace retrieval requires API key authentication. "
                    "Configure API_SERVER_KEY to enable this feature."
                ),
                status=403,
            )
        ...
        if db is None:
            return web.json_response(
                {"error": {"message": "Session database unavailable", "type": "server_error"}},
                status=503,
            )

        # Query workspace_state table directly (managed by NativeMemory)
        try:
            rows = db._conn.execute(
                "SELECT key, value, updated_at FROM workspace_state WHERE key IN (?, ?, ?, ?, ?)",
                ("files", "errors", "decisions", "goals", "last_task"),
            ).fetchall()
        except Exception as e:
            logger.warning("Workspace state query failed for %s: %s", session_id, e)
            return web.json_response(
                {"error": {"message": "Workspace state unavailable", "type": "server_error"}},
                status=503,
            )
        ...
        for row in rows:
            ...
            try:
                workspace[key] = json.loads(value)
            except (json.JSONDecodeError, TypeError):
                workspace[key] = value
            workspace[f"{key}_updated_at"] = updated_at
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Session workspace state (structured context for SaaS context bridge)
            self._app.router.add_get("/v1/sessions/{session_id}/workspace", self._handle_get_session_workspace)

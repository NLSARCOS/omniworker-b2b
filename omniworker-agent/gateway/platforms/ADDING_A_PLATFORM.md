...
There are two ways to add a platform to the OmniWorker gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.omniworker/plugins/` (or under `plugins/platforms/`
...
**zero changes to core OmniWorker code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `omniworker gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `omniworker_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the OmniWorker core.
...
And add it to the `omniworker-gateway` composite:

```python
"omniworker-gateway": {
    "includes": [..., "omniworker-your-platform"]
}
```
...
## 12. Status Display (`omniworker_cli/status.py`)
...
## 13. Gateway Setup Wizard (`omniworker_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ omniworker_cli/ toolsets.py \
  --include="*.py" -l | sort -u

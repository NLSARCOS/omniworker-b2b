...
There are two ways to add a platform to the Flux Agent gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.flux-agent/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Flux Agent code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `flux-agent gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `flux-agent_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Flux Agent core.
...
```python
"flux-agent-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _FLUX AGENT_CORE_TOOLS,
    "includes": []
},
```

And add it to the `flux-agent-gateway` composite:

```python
"flux-agent-gateway": {
    "includes": [..., "flux-agent-your-platform"]
}
```
...
## 12. Status Display (`flux-agent_cli/status.py`)
...
## 13. Gateway Setup Wizard (`flux-agent_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ flux-agent_cli/ toolsets.py \
  --include="*.py" -l | sort -u

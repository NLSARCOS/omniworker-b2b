...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        flux-agent_home_str = os.environ.get("OMNIWORKER_HOME")
        if flux-agent_home_str:
            takeover_path = Path(flux-agent_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(flux-agent_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    # constant in flux-agent_cli/gateway.py.
    headroom = 30.0

...
version: 0.1.0
author: Hugo Sequier (Hugo-SEQUIER), Flux Agent Agent
license: MIT
...
The script reads `~/.flux-agent/.env` for two optional defaults:
...
Helper script: `~/.flux-agent/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python3 ~/.flux-agent/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `~/.flux-agent/.env`.
...
```bash
python3 ~/.flux-agent/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python3 ~/.flux-agent/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \

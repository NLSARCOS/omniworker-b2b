...
The server-building part may be correct while the Flux Agent config is not. Load the `native-mcp` skill and configure the server in `~/.omniworker/config.yaml`, then restart Flux Agent.

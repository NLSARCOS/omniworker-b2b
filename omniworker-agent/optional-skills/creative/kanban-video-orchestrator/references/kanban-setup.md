...
the actual `setup.sh` that creates the project workspace, configures OmniWorker
...
> [OmniWorker/kanban-video-pipeline](https://github.com/OmniWorker/kanban-video-pipeline).
...
2. **Create profiles** — `omniworker profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.omniworker/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `omniworker kanban create` assigned to the director
...
```bash
omniworker profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.omniworker/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.omniworker/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most OmniWorker
...
Each profile gets a `SOUL.md` at `~/.omniworker/profiles/<name>/SOUL.md` that
...
```bash
omniworker kanban create "Direct production of <video title>" \
    --assignee director \
...
`~/.omniworker/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.omniworker/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.omniworker/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.omniworker/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY omniworker ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY omniworker OPENROUTER_API_KEY || exit 1
# ...
```

...
Maps each role archetype to the Flux Agent skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public flux-agent-agent
...
## Flux Agent skills relevant to video production

### Visual / rendering skills (`flux-agent-agent/skills/creative/`)
...
### Design / pre-production skills (`flux-agent-agent/skills/creative/`)
...
### Audio / media skills (`flux-agent-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`flux-agent-agent/skills/devops/`)
...
These are **not** Flux Agent skills but external CLIs / APIs that profiles invoke.
...
## Built-in Flux Agent tools for media review

These are native Flux Agent tools — not invoked via terminal but through their own
...
key is present in `~/.flux-agent/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Flux Agent profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.flux-agent/.env` → environment variable.
...
When a new Flux Agent-public video skill ships:

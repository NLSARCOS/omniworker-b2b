...
Maps each role archetype to the OmniWorker skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public omniworker-agent
...
## OmniWorker skills relevant to video production

### Visual / rendering skills (`omniworker-agent/skills/creative/`)
...
### Design / pre-production skills (`omniworker-agent/skills/creative/`)
...
### Audio / media skills (`omniworker-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`omniworker-agent/skills/devops/`)
...
These are **not** OmniWorker skills but external CLIs / APIs that profiles invoke.
...
## Built-in OmniWorker tools for media review

These are native OmniWorker tools — not invoked via terminal but through their own
...
key is present in `~/.omniworker/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every OmniWorker profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.omniworker/.env` → environment variable.
...
When a new OmniWorker-public video skill ships:

# Flux Agent Agent Security Policy

This document describes Flux Agent Agent's trust model, names the one
...
Flux Agent Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Flux Agent Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Flux Agent Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Flux Agent Agent's documentation
...
Flux Agent Agent supports two OS-level isolation postures. They address
...
Flux Agent Agent supports this in two ways:

- **Flux Agent Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Flux Agent Agent's in-process heuristics
...
Flux Agent Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Flux Agent Agent
itself. Bugs in Flux Agent Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Flux Agent Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Flux Agent Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Flux Agent Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Flux Agent Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Flux Agent Agent. Bugs in Flux Agent
...
- Flux Agent Agent includes supply-chain guards for MCP server

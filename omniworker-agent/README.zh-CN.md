...
> **Android / Termux：** 已测试的手动安装路径请参考 [Termux 指南](https://omniworker-agent.omniworker.com/docs/getting-started/termux)。在 Termux 上，OmniWorker 会安装精选的 `.[termux]` 扩展，因为完整的 `.[all]` 扩展会拉取 Android 不兼容的语音依赖。
>
...
omniworker claw migrate # 从 OpenClaw 迁移（如果来自 OpenClaw）
...
OmniWorker 有两种入口：用 `omniworker` 启动终端 UI，或运行网关从 Telegram、Discord、Slack、WhatsApp、Signal 或 Email 与之对话。进入对话后，许多斜杠命令在两种界面中通用。
...
如果你来自 OpenClaw，OmniWorker 可以自动导入你的设置、记忆、技能和 API 密钥。

**首次安装时：** 安装向导（`omniworker setup`）会自动检测 `~/.openclaw` 并在配置开始前提供迁移选项。
...
- **技能** — 用户创建的技能 → `~/.omniworker/skills/openclaw-imports/`
- **命令白名单** — 审批模式
- **消息设置** — 平台配置、允许用户、工作目录
...
使用 `omniworker claw migrate --help` 查看所有选项，或使用 `openclaw-migration` 技能进行交互式代理引导迁移（含干运行预览）。
...
- 🐛 [问题反馈](https://github.com/OmniWorker/omniworker-agent/issues)
- 💡 [讨论区](https://github.com/OmniWorker/omniworker-agent/discussions)
- 🔌 [OmniWorkerClaw](https://github.com/AaronWong1999/omniworkerclaw) — 社区微信桥接：在同一微信账号上运行 OmniWorker Agent 和 OpenClaw。

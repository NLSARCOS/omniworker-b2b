...
Resolution order for config file:
  1. $OMNIWORKER_HOME/honcho.json  (instance-local, enables isolated OmniWorker instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active OmniWorker profile.
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into OmniWorker system prompt
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active OmniWorker profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. OmniWorker session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — OmniWorker session_id ({timestamp}_{hex})
        ...
        # per-session: inherit OmniWorker session_id (new Honcho session each run)

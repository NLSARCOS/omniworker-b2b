...
# ``model_tools`` import time, so ``flux-agent status``, ``flux-agent chat``, etc.
...
# ``flux-agent_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Flux Agent is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a flux-agent MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${FLUX AGENT_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _flux-agent_home = _get_flux-agent_home()
        except (ModuleNotFoundError, ImportError):
            _flux-agent_home = _Path.home() / ".flux-agent"
        self._thread_count_store = _ThreadCountStore(
            _flux-agent_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Flux Agent is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("FLUX AGENT_HOME", str(_Path.home() / ".flux-agent"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${FLUX AGENT_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a flux-agent MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Flux Agent is thinking…' marker message.
        ...
        "Flux Agent is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Flux Agent is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Flux Agent is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Flux Agent is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Flux Agent is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``flux-agent setup``.

    The setup wizard at ``flux-agent_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.flux-agent/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. flux-agent-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.flux-agent/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'flux-agent-agent[google_chat]'",
        ...
        # the "Flux Agent is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Flux Agent is thinking…' marker message "

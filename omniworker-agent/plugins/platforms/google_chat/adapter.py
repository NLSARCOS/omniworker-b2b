...
# ``model_tools`` import time, so ``omniworker status``, ``omniworker chat``, etc.
...
# ``omniworker_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "OmniWorker is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a omniworker MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${OMNIWORKER_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _omniworker_home = _get_omniworker_home()
        except (ModuleNotFoundError, ImportError):
            _omniworker_home = _Path.home() / ".omniworker"
        self._thread_count_store = _ThreadCountStore(
            _omniworker_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "OmniWorker is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("OMNIWORKER_HOME", str(_Path.home() / ".omniworker"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${OMNIWORKER_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a omniworker MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'OmniWorker is thinking…' marker message.
        ...
        "OmniWorker is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "OmniWorker is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "OmniWorker is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "OmniWorker is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "OmniWorker is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``omniworker setup``.

    The setup wizard at ``omniworker_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.omniworker/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. omniworker-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.omniworker/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'omniworker-agent[google_chat]'",
        ...
        # the "OmniWorker is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'OmniWorker is thinking…' marker message "

...
``_check_spotify_available()`` — when the user has not run ``flux-agent auth
spotify``, the tools remain registered (so they appear in ``flux-agent
...
The Spotify auth flow (``flux-agent auth spotify``), CLI plumbing, and docs

...
``_check_spotify_available()`` — when the user has not run ``omniworker auth
spotify``, the tools remain registered (so they appear in ``omniworker
...
The Spotify auth flow (``omniworker auth spotify``), CLI plumbing, and docs

/**
 * OmniWorker Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.omniworker/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__OMNIWORKER_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__OMNIWORKER_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "omniworker-kanban-dot-triage",
    todo: "omniworker-kanban-dot-todo",
    ready: "omniworker-kanban-dot-ready",
    running: "omniworker-kanban-dot-running",
    blocked: "omniworker-kanban-dot-blocked",
    done: "omniworker-kanban-dot-done",
    archived: "omniworker-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-omniworker-task";
  ...
  const DOCS_URL = "https://omniworker-agent.omniworker.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://omniworker-agent.omniworker.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "omniworker.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="omniworker-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "omniworker-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "omniworker-kanban-md",
  ...
  // standard `drop` event and our `omniworker-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("omniworker-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (col !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("omniworker-kanban-column--drop");
          if (col) col.classList.add("omniworker-kanban-column--drop");
          lastTarget = col;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("omniworker-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          lastTarget.dispatchEvent(new CustomEvent("omniworker-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__OMNIWORKER_SESSION_TOKEN__ || "";
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "omniworker-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "omniworker-kanban-attention",
        "omniworker-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "omniworker-kanban-attention-bar" },
        h("span", { className: "omniworker-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "omniworker-kanban-attention-text" },
          diagTasks.length === 1
        ...
        h("button", {
          className: "omniworker-kanban-attention-toggle",
          onClick: function () { setExpanded(function (x) { return !x; }); },
          type: "button",
        }, expanded ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
        h("button", {
          className: "omniworker-kanban-attention-dismiss",
      ...
      expanded
        ? h("div", { className: "omniworker-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "omniworker-kanban-attention-row",
                  "omniworker-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "omniworker-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "omniworker-kanban-attention-row-id" }, task.id),
                h("span", { className: "omniworker-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "omniworker-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "omniworker-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "omniworker-kanban-diag-action-btn",
      action.suggested ? "omniworker-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " omniworker-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".omniworker-kanban-drawer-comment-row input, .omniworker-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "omniworker-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("omniworker-kanban-diag", sevClass) },
      h("div", { className: "omniworker-kanban-diag-header" },
        h("span", { className: "omniworker-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "omniworker-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "omniworker-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "omniworker-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "omniworker-kanban-diag-data-row" },
                  h("span", { className: "omniworker-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "omniworker-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "omniworker-kanban-diag-data-row" },
                h("span", { className: "omniworker-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "omniworker-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "omniworker-kanban-diag-reassign-row" },
            h("span", { className: "omniworker-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "omniworker-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "omniworker-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "omniworker-kanban-diag-msg",
              msg.ok ? "omniworker-kanban-diag-msg--ok" : "omniworker-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "omniworker-kanban-section" },
      h("div", { className: "omniworker-kanban-section-head-row" },
        h("span", { className: "omniworker-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "omniworker-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "omniworker-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "omniworker-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "omniworker-kanban-docs-link",
      title: "Open OmniWorker Kanban docs in a new tab",
      "aria-label": "OmniWorker Kanban documentation",
    }, "?");
  }
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "omniworker-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "omniworker-kanban-boardswitcher" },
      h("div", { className: "omniworker-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "omniworker-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "omniworker-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "omniworker-kanban-dialog-title" },
        ...
        h("div", { className: "omniworker-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned OmniWorker profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "omniworker-kanban-bulk" },
      h("span", { className: "omniworker-kanban-bulk-count" },
      ...
      h("div", { className: "omniworker-kanban-bulk-priority",
      ...
      h("div", { className: "omniworker-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different OmniWorker profile. Pick a profile (or unassign) and click Apply." },
        h(Select, {
          value: assignee,
      ...
      h("label", { className: "omniworker-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h("input", {
          type: "checkbox",
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".omniworker-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "omniworker-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("omniworker-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("omniworker-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "omniworker-kanban-column",
        dragOver ? "omniworker-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "omniworker-kanban-column-header",
                 title: colHelp || "" },
        h("input", {
          type: "checkbox",
          className: "omniworker-kanban-col-check",
        ...
        h("span", { className: cn("omniworker-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "omniworker-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "omniworker-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "omniworker-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "omniworker-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "omniworker-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "omniworker-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "omniworker-kanban-lane" },
                  h("div", { className: "omniworker-kanban-lane-head" },
                    h("span", { className: "omniworker-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "omniworker-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "omniworker-kanban-card--stale-red";
    if (age >= tier.amber) return "omniworker-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".omniworker-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "omniworker-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "omniworker-kanban-card",
        props.selected ? "omniworker-kanban-card--selected" : "",
        props.failed ? "omniworker-kanban-card--failed" : "",
        props.draggingSource ? "omniworker-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "omniworker-kanban-card-content" },
          h("div", { className: "omniworker-kanban-card-row" },
            h("label", {
              className: "omniworker-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h("input", {
                type: "checkbox",
                className: "omniworker-kanban-card-check",
            ...
            h("span", { className: "omniworker-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or omniworker kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "omniworker-kanban-warning-badge",
                    "omniworker-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
              ...
              : null,
            t.priority > 0
              ? h(Badge, { className: "omniworker-kanban-priority",
                           title: `Priority ${t.priority}. Higher-priority tasks are claimed first by the dispatcher.` }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "omniworker-kanban-tag",
                           title: `Tenant: ${t.tenant}. Free-form tag for grouping tasks (customer, project, team).` }, t.tenant)
              : null,
            progress
              ? h("span", {
                  className: cn(
                    "omniworker-kanban-progress",
                    progress.done === progress.total ? "omniworker-kanban-progress--full" : "",
                  ),
                  title: `${progress.done} of ${progress.total} child tasks done`,
                }, `${progress.done}/${progress.total}`)
              : null,
          ),
          h("div", { className: "omniworker-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "omniworker-kanban-card-row omniworker-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "omniworker-kanban-assignee",
                            title: `Assigned to OmniWorker profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "omniworker-kanban-unassigned",
            ...
            t.comment_count > 0
              ? h("span", { className: "omniworker-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "omniworker-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "omniworker-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "OmniWorker profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "OmniWorker profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "omniworker-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "omniworker-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "omniworker-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "omniworker-kanban-drawer-close",
        ...
        data ? h("div", { className: "omniworker-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "omniworker-kanban-drawer-body" },
      h("div", { className: "omniworker-kanban-drawer-title" },
        h("span", { className: cn("omniworker-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "omniworker-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "omniworker-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "omniworker-kanban-section" },
        h("div", { className: "omniworker-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "omniworker-kanban-section" },
        h("div", { className: "omniworker-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "omniworker-kanban-comment" },
                h("div", { className: "omniworker-kanban-comment-head" },
                  h("span", { className: "omniworker-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "omniworker-kanban-comment-ago" },
      ...
      h("div", { className: "omniworker-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "omniworker-kanban-event",
              isDiag ? "omniworker-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "omniworker-kanban-event-header" },
                  h("span", { className: "omniworker-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "omniworker-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "omniworker-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "omniworker-kanban-event-header-plain" },
                  h("span", { className: "omniworker-kanban-event-kind" }, e.kind),
                  h("span", { className: "omniworker-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "omniworker-kanban-event-phantom-row" },
                  h("span", { className: "omniworker-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "omniworker-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "omniworker-kanban-event-payload" },
    ...
    return h("div", { className: "omniworker-kanban-section" },
      h("div", { className: "omniworker-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              ...
              className: "omniworker-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `omniworker-kanban-run--${r.outcome || r.status || "ended"}`
          : "omniworker-kanban-run--active";
        return h("div", { key: r.id, className: cn("omniworker-kanban-run", outcomeClass) },
          h("div", { className: "omniworker-kanban-run-head" },
            h("span", { className: "omniworker-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "omniworker-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "omniworker-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "omniworker-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "omniworker-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "omniworker-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "omniworker-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "omniworker-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "omniworker-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "omniworker-kanban-pre omniworker-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "omniworker-kanban-meta-row" },
      h("span", { className: "omniworker-kanban-meta-label" }, props.label),
      h("span", { className: "omniworker-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "omniworker-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "omniworker-kanban-meta-row" },
        h("span", { className: "omniworker-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "omniworker-kanban-meta-value omniworker-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "omniworker-kanban-meta-row" },
        h("span", { className: "omniworker-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "omniworker-kanban-meta-value omniworker-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "omniworker-kanban-section" },
      h("div", { className: "omniworker-kanban-section-head-row" },
        h("span", { className: "omniworker-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "omniworker-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "omniworker-kanban-section" },
      h("div", { className: "omniworker-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "omniworker-kanban-deps-row" },
        h("span", { className: "omniworker-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "omniworker-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "omniworker-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "omniworker-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "omniworker-kanban-dep-chip-x",
      ...
      h("div", { className: "omniworker-kanban-deps-row" },
        h("span", { className: "omniworker-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "omniworker-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "omniworker-kanban-actions" },
        specifyButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "omniworker-kanban-msg-ok"
          : "omniworker-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "omniworker-kanban-section" },
      ...
      h("div", { className: "omniworker-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "omniworker-kanban-home-sub omniworker-kanban-home-sub--on"
              : "omniworker-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__OMNIWORKER_PLUGINS__ && typeof window.__OMNIWORKER_PLUGINS__.register === "function") {
    window.__OMNIWORKER_PLUGINS__.register("kanban", KanbanPage);
  }
})();

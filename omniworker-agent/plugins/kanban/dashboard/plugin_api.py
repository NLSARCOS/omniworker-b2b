...
``omniworker_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``omniworker_cli/web_server.py``.

This means ``omniworker dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from omniworker_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``omniworker_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from omniworker_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from omniworker_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``omniworker kanban
...
# spawn failures, stuck-blocked). See omniworker_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``omniworker kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``omniworker kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``omniworker kanban specify <task_id>``.
    ...
    try:
        ...
        from omniworker_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``omniworker kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.omniworker/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.omniworker/profiles/*`` on disk and every
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in omniworker_cli/web_server.py.

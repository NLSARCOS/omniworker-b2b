...
Auto-tracks and cleans up ephemeral files created during OmniWorker Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$OMNIWORKER_HOME` and `/tmp/omniworker-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `OMNIWORKER_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `OMNIWORKER_HOME` or `/tmp/omniworker-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$OMNIWORKER_HOME/disk-cleanup/` is itself excluded
- `$OMNIWORKER_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

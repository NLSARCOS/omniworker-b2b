...
Scoped strictly to `$FLUX AGENT_HOME` and `/tmp/flux-agent-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `FLUX AGENT_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `FLUX AGENT_HOME` or `/tmp/flux-agent-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$FLUX AGENT_HOME/disk-cleanup/` is itself excluded
- `$FLUX AGENT_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

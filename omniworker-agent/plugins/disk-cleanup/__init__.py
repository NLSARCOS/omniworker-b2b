...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``FLUX AGENT_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$FLUX AGENT_HOME/disk-cleanup/cleanup.log``.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        # Tokenise the command — catches `touch /tmp/flux-agent-x/test_foo.py`
        try:
...
All operations are scoped to FLUX AGENT_HOME and /tmp/flux-agent-*.
...
def _handle_slash(raw_args: str) -> Optional[str]:
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside FLUX AGENT_HOME): "
            f"{path_arg}"
        )

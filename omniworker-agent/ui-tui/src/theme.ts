...
// Terminal. Explicit OMNIWORKER_TUI_THEME / COLORFGBG signals above still win,
...
// `OMNIWORKER_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `OMNIWORKER_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `OMNIWORKER_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `OMNIWORKER_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default OmniWorker palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.OMNIWORKER_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.OMNIWORKER_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.OMNIWORKER_TUI_BACKGROUND ?? '')

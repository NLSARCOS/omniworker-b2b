...
await build({
  ...
  // Skip the prebuilt @omniworker/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@omniworker/ink': resolve(root, 'packages/omniworker-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The omniworker_cli launcher

...
React + Ink terminal UI for Flux Agent. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `FLUX AGENT_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.flux-agent/.flux-agent_history` or under `FLUX AGENT_HOME`.
...
```text
ui-tui/
  packages/flux-agent-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      flux-agent-ink.d.ts    type declarations for @flux-agent/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent Flux AgentCLI subprocess for slash commands
```

...
React + Ink terminal UI for OmniWorker. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `OMNIWORKER_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.omniworker/.omniworker_history` or under `OMNIWORKER_HOME`.
...
```text
ui-tui/
  packages/omniworker-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      omniworker-ink.d.ts    type declarations for @omniworker/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent OmniWorkerCLI subprocess for slash commands
```

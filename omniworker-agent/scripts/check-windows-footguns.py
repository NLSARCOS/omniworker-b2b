...
FOOTGUNS: list[Footgun] = [
    ...
    Footgun(
        ...
        fix=(
            ...
            "Or gateway.status._pid_exists(pid) for the flux-agent wrapper "
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        "--all",
        action="store_true",
        help="Scan the full repository (flux-agent_cli/, gateway/, tools/, cron/, etc.).",
    )
    p.add_argument(
        "--diff",
        metavar="REF",

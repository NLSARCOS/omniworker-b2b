#!/usr/bin/env bash
# Canonical test runner for omniworker-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.omniworker/omniworker-agent/venv"; do
...
# Unset OMNIWORKER_* behavioral vars too.
unset OMNIWORKER_YOLO_MODE OMNIWORKER_INTERACTIVE OMNIWORKER_QUIET OMNIWORKER_TOOL_PROGRESS \
      OMNIWORKER_TOOL_PROGRESS_MODE OMNIWORKER_MAX_ITERATIONS OMNIWORKER_SESSION_PLATFORM \
      OMNIWORKER_SESSION_CHAT_ID OMNIWORKER_SESSION_CHAT_NAME OMNIWORKER_SESSION_THREAD_ID \
      OMNIWORKER_SESSION_SOURCE OMNIWORKER_SESSION_KEY OMNIWORKER_GATEWAY_SESSION \
      OMNIWORKER_CRON_SESSION \
      OMNIWORKER_PLATFORM OMNIWORKER_INFERENCE_PROVIDER OMNIWORKER_MANAGED OMNIWORKER_DEV \
      OMNIWORKER_CONTAINER OMNIWORKER_EPHEMERAL_SYSTEM_PROMPT OMNIWORKER_TIMEZONE \
      OMNIWORKER_REDACT_SECRETS OMNIWORKER_BACKGROUND_NOTIFICATIONS OMNIWORKER_EXEC_ASK \
      OMNIWORKER_HOME_MODE 2>/dev/null || true
...
# If a system-wide omniworker pytest_live_guard plugin is installed at
# $HOME/.omniworker/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.omniworker/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.omniworker:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.omniworker" ;;
  esac
...
WORKERS="${OMNIWORKER_TEST_WORKERS:-4}"

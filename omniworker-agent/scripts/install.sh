...
# Python-driven tool session (e.g. OmniWorker terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:OmniWorker/omniworker-agent.git"
REPO_URL_HTTPS="https://github.com/OmniWorker/omniworker-agent.git"
OMNIWORKER_HOME="${OMNIWORKER_HOME:-$HOME/.omniworker}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${OMNIWORKER_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$OMNIWORKER_INSTALL_DIR"
...
#   code at /usr/local/lib/omniworker-agent, command at /usr/local/bin/omniworker,
#   data still at /root/.omniworker (OMNIWORKER_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.omniworker/omniworker-agent"
            echo "                   default (root, Linux): /usr/local/lib/omniworker-agent"
            echo "  --omniworker-home PATH  Data directory (default: ~/.omniworker, or \$OMNIWORKER_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, OmniWorker installs the code under"
            echo "  /usr/local/lib/omniworker-agent and links the command into"
            echo "  /usr/local/bin/omniworker (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$OMNIWORKER_HOME"
            echo "  (default /root/.omniworker).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$OMNIWORKER_HOME/omniworker-agent are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `omniworker` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $OMNIWORKER_HOME/omniworker-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $OMNIWORKER_HOME/omniworker-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/omniworker-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $OMNIWORKER_HOME/omniworker-agent — then preserve it)
#
# Always no-op when the user set --dir or $OMNIWORKER_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in OMNIWORKER_HOME.
    if is_termux; then
        INSTALL_DIR="$OMNIWORKER_HOME/omniworker-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$OMNIWORKER_HOME/omniworker-agent/.git" ]; then
            INSTALL_DIR="$OMNIWORKER_HOME/omniworker-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/omniworker-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/omniworker-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/omniworker"
        log_info "  Data:    $OMNIWORKER_HOME (unchanged)"
        return 0
    fi
...
get_omniworker_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/omniworker" ]; then
        echo "$link_dir/omniworker"
    else
        echo "omniworker"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/OmniWorker/omniworker-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/omniworker-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/omniworker-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$OMNIWORKER_HOME/node/bin/node" ]; then
        export PATH="$OMNIWORKER_HOME/node/bin:$PATH"
        local found_ver=$("$OMNIWORKER_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (OmniWorker-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.omniworker/node/..."
    ...
    # Place into ~/.omniworker/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$OMNIWORKER_HOME/node"
    mkdir -p "$OMNIWORKER_HOME"
    mv "$extracted_dir" "$OMNIWORKER_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$OMNIWORKER_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$OMNIWORKER_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$OMNIWORKER_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$OMNIWORKER_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$OMNIWORKER_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.omniworker/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. OmniWorker install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "OmniWorker Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="omniworker-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if OmniWorker behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"omniworker-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up omniworker command..."

    if [ "$USE_VENV" = true ]; then
        OMNIWORKER_BIN="$INSTALL_DIR/venv/bin/omniworker"
    else
        OMNIWORKER_BIN="$(which omniworker 2>/dev/null || echo "")"
        if [ -z "$OMNIWORKER_BIN" ]; then
            log_warn "omniworker not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$OMNIWORKER_BIN" ]; then
        log_warn "omniworker entry point not found at $OMNIWORKER_BIN"
    ...
    # Create a user-facing shim for the omniworker command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $OMNIWORKER_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$OMNIWORKER_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/omniworker"
    cat > "$command_link_dir/omniworker" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$OMNIWORKER_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/omniworker"
    log_success "Installed omniworker launcher → $command_link_display_dir/omniworker"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "omniworker command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v omniworker' \
                >/dev/null 2>&1; then
        ...
        log_info "omniworker not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# OmniWorker Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# OmniWorker Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# OmniWorker Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so omniworker works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.omniworker directory structure (config at top level, code in subdir)
    mkdir -p "$OMNIWORKER_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.omniworker/.env (top level, easy to find)
    if [ ! -f "$OMNIWORKER_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$OMNIWORKER_HOME/.env"
            log_success "Created ~/.omniworker/.env from template"
        else
            touch "$OMNIWORKER_HOME/.env"
            log_success "Created ~/.omniworker/.env"
        fi
    else
        log_info "~/.omniworker/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$OMNIWORKER_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.omniworker/config.yaml (top level, easy to find)
    if [ ! -f "$OMNIWORKER_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$OMNIWORKER_HOME/config.yaml"
            log_success "Created ~/.omniworker/config.yaml from template"
        fi
    else
        log_info "~/.omniworker/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$OMNIWORKER_HOME/SOUL.md" ]; then
        cat > "$OMNIWORKER_HOME/SOUL.md" << 'SOUL_EOF'
# OmniWorker Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how OmniWorker communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.omniworker/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.omniworker/"

    # Seed bundled skills into ~/.omniworker/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.omniworker/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.omniworker/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$OMNIWORKER_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$OMNIWORKER_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.omniworker/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$OMNIWORKER_HOME/.env"
    ...
    {
        echo ""
        echo "# OmniWorker Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; OmniWorker will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (omniworker --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'omniworker setup' after install."
        return 0
    fi
    ...
    # Run omniworker setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m omniworker_cli.main setup < /dev/tty
    else
        python -m omniworker_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for OmniWorker to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$OMNIWORKER_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'omniworker whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                OMNIWORKER_CMD="$(get_omniworker_command_path)"
                $OMNIWORKER_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'omniworker whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'omniworker gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $OMNIWORKER_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $OMNIWORKER_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: omniworker gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: omniworker gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $OMNIWORKER_CMD gateway > "$OMNIWORKER_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.omniworker/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: omniworker gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: omniworker gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $OMNIWORKER_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $OMNIWORKER_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $OMNIWORKER_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}omniworker${NC}              Start chatting"
    echo -e "   ${GREEN}omniworker setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}omniworker config${NC}       View/edit configuration"
    echo -e "   ${GREEN}omniworker config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}omniworker gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}omniworker update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'omniworker' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'omniworker' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'omniworker' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"

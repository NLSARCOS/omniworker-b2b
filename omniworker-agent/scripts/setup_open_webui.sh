...
# Bootstrap Open WebUI against OmniWorker Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.omniworker/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-omniworker.sh
...
#   OPEN_WEBUI_NAME='Johnny OmniWorker'
...
#   OMNIWORKER_API_PORT=8642
#   OMNIWORKER_API_HOST=127.0.0.1
#   OMNIWORKER_API_MODEL_NAME='OmniWorker Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-OmniWorker Agent WebUI}"
...
OMNIWORKER_ENV_FILE="${OMNIWORKER_ENV_FILE:-$HOME/.omniworker/.env}"
OMNIWORKER_API_PORT="${OMNIWORKER_API_PORT:-8642}"
OMNIWORKER_API_HOST="${OMNIWORKER_API_HOST:-127.0.0.1}"
OMNIWORKER_API_CONNECT_HOST="${OMNIWORKER_API_CONNECT_HOST:-127.0.0.1}"
OMNIWORKER_API_MODEL_NAME="${OMNIWORKER_API_MODEL_NAME:-OmniWorker Agent}"
OMNIWORKER_API_BASE_URL="http://${OMNIWORKER_API_CONNECT_HOST}:${OMNIWORKER_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-omniworker.sh"
LOG_DIR="$HOME/.omniworker/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$OMNIWORKER_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$OMNIWORKER_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.omniworker.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.omniworker</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.omniworker"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.omniworker"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-omniworker.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to OmniWorker Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-omniworker.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.omniworker/logs/openwebui.log
StandardError=append:%h/.omniworker/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-omniworker.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$OMNIWORKER_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring OmniWorker API server is configured...'
  upsert_env API_SERVER_ENABLED true "$OMNIWORKER_ENV_FILE"
  upsert_env API_SERVER_HOST "$OMNIWORKER_API_HOST" "$OMNIWORKER_ENV_FILE"
  upsert_env API_SERVER_PORT "$OMNIWORKER_API_PORT" "$OMNIWORKER_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$OMNIWORKER_API_MODEL_NAME" "$OMNIWORKER_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$OMNIWORKER_ENV_FILE"
  ensure_env_permissions

  log 'Restarting OmniWorker gateway so API server settings take effect...'
  omniworker gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${OMNIWORKER_API_CONNECT_HOST}:${OMNIWORKER_API_PORT}/health" >/dev/null; then
    log 'OmniWorker API server did not answer on the first check. Trying to start gateway in the background...'
    nohup omniworker gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${OMNIWORKER_API_CONNECT_HOST}:${OMNIWORKER_API_PORT}/health" >/dev/null
  ...
  log "OmniWorker API endpoint: ${OMNIWORKER_API_BASE_URL}"

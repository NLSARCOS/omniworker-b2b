# ============================================================================
# OmniWorker Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$OmniWorkerHome = "$env:LOCALAPPDATA\omniworker",
    [string]$InstallDir = "$env:LOCALAPPDATA\omniworker\omniworker-agent"
)
...
$RepoUrlSsh = "git@github.com:OmniWorker/omniworker-agent.git"
$RepoUrlHttps = "https://github.com/OmniWorker/omniworker-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Install-Git {
    ...
    which OmniWorker uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\omniworker\git`` — never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    OmniWorker needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the OmniWorker copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\omniworker\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``OMNIWORKER_GIT_BASH_PATH`` (User scope) so OmniWorker can find it in a fresh
    ...
    # Download PortableGit into $OmniWorkerHome\git.  Always works as long as
    # we can reach github.com — no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found — downloading PortableGit to $OmniWorkerHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "omniworker-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected — PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent OmniWorker features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  OmniWorker needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``OMNIWORKER_GIT_BASH_PATH`` (User env scope) so OmniWorker can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $OmniWorkerHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $OmniWorkerHome\git\usr\bin\bash.exe
    $candidates += "$OmniWorkerHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$OmniWorkerHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("OMNIWORKER_GIT_BASH_PATH", $candidate, "User")
            $env:OMNIWORKER_GIT_BASH_PATH = $candidate
            Write-Info "Set OMNIWORKER_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe — OmniWorker may not find Git Bash."
    Write-Info "If needed, set OMNIWORKER_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$OmniWorkerHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$OmniWorkerHome\node;$env:Path"
        Write-Success "Node.js $version found (OmniWorker-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.omniworker/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\omniworker-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$OmniWorkerHome\node") { Remove-Item -Recurse -Force "$OmniWorkerHome\node" }
                Move-Item $extractedDir.FullName "$OmniWorkerHome\node"
                $env:Path = "$OmniWorkerHome\node;$env:Path"

                $version = & "$OmniWorkerHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.omniworker/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running omniworker processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/OmniWorker/omniworker-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\omniworker-agent-$Branch.zip"
                $extractPath = "$env:TEMP\omniworker-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `omniworker.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install omniworker-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and omniworker.exe broken with
    ...
    # here, not 30 seconds later when the user runs `omniworker`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `omniworker dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable — `omniworker dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `omniworker dashboard` should now work."
            } else {
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        $omniworkerBin = "$InstallDir"
    } else {
        $omniworkerBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so omniworker is globally available
    # On Windows, the omniworker.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$omniworkerBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$omniworkerBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $omniworkerBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set OMNIWORKER_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\omniworker instead
    # of the Unix default ~/.omniworker
    $currentOmniWorkerHome = [Environment]::GetEnvironmentVariable("OMNIWORKER_HOME", "User")
    if (-not $currentOmniWorkerHome -or $currentOmniWorkerHome -ne $OmniWorkerHome) {
        [Environment]::SetEnvironmentVariable("OMNIWORKER_HOME", $OmniWorkerHome, "User")
        Write-Success "Set OMNIWORKER_HOME=$OmniWorkerHome"
    }
    $env:OMNIWORKER_HOME = $OmniWorkerHome
    
    # Update current session
    $env:Path = "$omniworkerBin;$env:Path"
    
    Write-Success "omniworker command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.omniworker directory structure
    New-Item -ItemType Directory -Force -Path "$OmniWorkerHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OmniWorkerHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OmniWorkerHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OmniWorkerHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OmniWorkerHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OmniWorkerHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OmniWorkerHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OmniWorkerHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OmniWorkerHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$OmniWorkerHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.omniworker/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.omniworker/.env"
        }
    } else {
        Write-Info "~/.omniworker/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$OmniWorkerHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.omniworker/config.yaml from template"
        }
    } else {
        Write-Info "~/.omniworker/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and OmniWorker's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$OmniWorkerHome\SOUL.md"
...
"@
        ...
        Write-Success "Created ~/.omniworker/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.omniworker/"
    
    # Seed bundled skills into ~/.omniworker/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.omniworker/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.omniworker/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$OmniWorkerHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.omniworker/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'omniworker setup tools' later."
        return
    }
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\omniworker-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\omniworker-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
                try {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\omniworker-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.omniworker/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m omniworker_cli.main setup
    } else {
...
function Start-GatewayIfConfigured {
    ...
    $omniworkerCmd = "$InstallDir\venv\Scripts\omniworker.exe"
    if (-not (Test-Path $omniworkerCmd)) {
        $omniworkerCmd = "omniworker"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$OmniWorkerHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'omniworker whatsapp' to pair via QR code..."
        Write-Host ""
        $response = Read-Host "Pair WhatsApp now? [Y/n]"
        if ($response -eq "" -or $response -match "^[Yy]") {
            try {
                & $omniworkerCmd whatsapp
            } catch {
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$OmniWorkerHome\logs\gateway.log"
            Start-Process -FilePath $omniworkerCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$OmniWorkerHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: omniworker gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: omniworker gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$OmniWorkerHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$OmniWorkerHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$OmniWorkerHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$OmniWorkerHome\omniworker-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   omniworker              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   omniworker setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   omniworker config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   omniworker config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   omniworker gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   omniworker update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/OmniWorker/omniworker-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow

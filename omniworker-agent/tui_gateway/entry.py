...
# omniworker modules.  omniworker_cli sets OMNIWORKER_PYTHON_SRC_ROOT before spawning this
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `OMNIWORKER_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("OMNIWORKER_TUI_SIDECAR_URL")
...
# tested; override via ``OMNIWORKER_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("OMNIWORKER_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``OMNIWORKER_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``omniworker --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)

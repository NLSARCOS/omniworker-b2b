# nix/nixosModules.nix — NixOS module for omniworker-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: omniworker runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $OMNIWORKER_HOME/.env
# and read by omniworker at startup — no container recreation needed for env changes.
#
# Tool resolution: the omniworker wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.omniworker-agent;
    effectivePackage =
    ...
    omniworker-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-omniworker-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "omniworker-config-attrs";
      description = "OmniWorker YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "omniworker-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "omniworker-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'OMNIWORKER_DOC_EOF'\n${value}\nOMNIWORKER_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "omniworker-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/omniworker";
    ...
    # omniworker user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "omniworker-container-entrypoint" ''
      set -eu

      OMNIWORKER_UID="''${OMNIWORKER_UID:?OMNIWORKER_UID must be set}"
      OMNIWORKER_GID="''${OMNIWORKER_GID:?OMNIWORKER_GID must be set}"

      # ── Group: ensure a group with GID=$OMNIWORKER_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$OMNIWORKER_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="omniworker"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$OMNIWORKER_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$OMNIWORKER_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$OMNIWORKER_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$OMNIWORKER_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="omniworker"
        TARGET_HOME="/home/omniworker"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$OMNIWORKER_UID" -g "$OMNIWORKER_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$OMNIWORKER_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$OMNIWORKER_UID:$OMNIWORKER_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure OMNIWORKER_HOME is owned by the target user.
      ...
      if [ -n "''${OMNIWORKER_HOME:-}" ] && [ -d "$OMNIWORKER_HOME" ]; then
        find "$OMNIWORKER_HOME" \! -user "$OMNIWORKER_UID" -exec chown "$OMNIWORKER_UID:$OMNIWORKER_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/omniworker-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/omniworker-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/omniworker ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/omniworker
        chmod 0440 /etc/sudoers.d/omniworker
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$OMNIWORKER_UID" --regid="$OMNIWORKER_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $OMNIWORKER_HOME/.env.
    ...
    # Default: /var/lib/omniworker/workspace → /data/workspace.
  ...
  in {
    options.services.omniworker-agent = with lib; {
      enable = mkEnableOption "OmniWorker Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = omniworker-agent;
        description = "The omniworker-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/omniworker";
        description = "State directory. Contains .omniworker/ subdir (OMNIWORKER_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative OmniWorker config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $OMNIWORKER_HOME/.env at activation time.
          OmniWorker reads this file on every startup via load_omniworker_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $OMNIWORKER_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $OMNIWORKER_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `omniworker gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the omniworker user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the omniworker plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. OmniWorker discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          omniworker_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as omniworker (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in omniworker-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the omniworker CLI to environment.systemPackages and export
          OMNIWORKER_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.omniworker symlink to the service
            stateDir. These users are automatically added to the omniworker group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.omniworker-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the omniworker CLI to system PATH and export OMNIWORKER_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.omniworker/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.OMNIWORKER_HOME = "${cfg.stateDir}/.omniworker";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.omniworker-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the omniworker user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.omniworker-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed omniworker
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure omniworker is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.omniworker        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.omniworker/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.omniworker/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.omniworker/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.omniworker/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.omniworker/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."omniworker-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.omniworker
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.omniworker ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.omniworker ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.omniworker -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.omniworker/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.omniworker/$_subdir"
            chmod 2770 "${cfg.stateDir}/.omniworker/$_subdir"
            find "${cfg.stateDir}/.omniworker/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.omniworker/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.omniworker/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.omniworker/config.yaml
            chmod 0640 ${cfg.stateDir}/.omniworker/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.omniworker/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.omniworker/.managed
          chmod 0644 ${cfg.stateDir}/.omniworker/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.omniworker/.container-mode <<'OMNIWORKER_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    omniworker_bin=${containerDataDir}/current-package/bin/omniworker
    OMNIWORKER_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.omniworker/.container-mode
            chmod 0644 ${cfg.stateDir}/.omniworker/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.omniworker/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.omniworker";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.omniworker" ]; then
                  rm -f "${symlinkPath}"
                  echo "omniworker-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.omniworker -> stateDir/.omniworker for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.omniworker";
                target = "${cfg.stateDir}/.omniworker";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "omniworker-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.omniworker/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.omniworker/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.omniworker/auth.json
              fi
            ''}
          ''}
          ...
          # OmniWorker reads $OMNIWORKER_HOME/.env at startup via load_omniworker_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.omniworker/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'OMNIWORKER_NIX_ENV_EOF'
    ${envFileContent}
    OMNIWORKER_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
        ...
        find ${cfg.stateDir}/.omniworker/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.omniworker/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.omniworker/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.omniworker-agent = {
          description = "OmniWorker Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            OMNIWORKER_HOME = "${cfg.stateDir}/.omniworker";
            OMNIWORKER_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $OMNIWORKER_HOME/.env by the activation script. load_omniworker_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/omniworker"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the omniworker group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.omniworker-agent = {
          description = "OmniWorker Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              OMNIWORKER_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              OMNIWORKER_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env OMNIWORKER_UID="$OMNIWORKER_UID" \
                --env OMNIWORKER_GID="$OMNIWORKER_GID" \
                --env OMNIWORKER_HOME=${containerDataDir}/.omniworker \
                --env OMNIWORKER_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/omniworker gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}

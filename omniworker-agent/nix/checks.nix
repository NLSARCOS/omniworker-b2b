...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      omniworker-agent = self'.packages.default;
      omniworkerVenv = omniworker-agent.omniworkerVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "omniworker-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${omniworkerVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "omniworker-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "omniworker-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${omniworker-agent}/bin/omniworker || (echo "FAIL: omniworker binary missing"; exit 1)
          test -x ${omniworker-agent}/bin/omniworker-agent || (echo "FAIL: omniworker-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${omniworker-agent}/bin/omniworker version 2>&1 | grep -qi "omniworker" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "omniworker-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in omniworker omniworker-agent omniworker-acp; do
            test -x ${omniworker-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "omniworker-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking omniworker --help ==="
          ${omniworker-agent}/bin/omniworker --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${omniworker-agent}/bin/omniworker --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "omniworker-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${omniworker-agent}/share/omniworker-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${omniworker-agent}/share/omniworker-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "OMNIWORKER_BUNDLED_SKILLS" ${omniworker-agent}/bin/omniworker || \
            (echo "FAIL: OMNIWORKER_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: OMNIWORKER_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "omniworker-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${omniworker-agent}/share/omniworker-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${omniworker-agent}/share/omniworker-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "OMNIWORKER_BUNDLED_PLUGINS" ${omniworker-agent}/bin/omniworker || \
            (echo "FAIL: OMNIWORKER_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: OMNIWORKER_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "omniworker-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${omniworker-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${omniworker-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "OMNIWORKER_TUI_DIR" ${omniworker-agent}/bin/omniworker || \
            (echo "FAIL: OMNIWORKER_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: OMNIWORKER_TUI_DIR set in wrapper"
        ...
        # Verify OMNIWORKER_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        omniworker-node = pkgs.runCommand "omniworker-node-version" { } ''
          set -e
          echo "=== Checking OMNIWORKER_NODE in wrapper ==="
          grep -q "OMNIWORKER_NODE" ${omniworker-agent}/bin/omniworker || \
            (echo "FAIL: OMNIWORKER_NODE not set in wrapper"; exit 1)
          echo "PASS: OMNIWORKER_NODE present in wrapper"

          OMNIWORKER_NODE=$(sed -n "s/^export OMNIWORKER_NODE='\(.*\)'/\1/p" ${omniworker-agent}/bin/omniworker)
          test -x "$OMNIWORKER_NODE" || (echo "FAIL: OMNIWORKER_NODE=$OMNIWORKER_NODE not executable"; exit 1)
          echo "PASS: OMNIWORKER_NODE executable at $OMNIWORKER_NODE"

          NODE_MAJOR=$("$OMNIWORKER_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All OMNIWORKER_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify OMNIWORKER_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "omniworker-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(OMNIWORKER_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking OMNIWORKER_MANAGED guards ==="
          check_blocked "config set" ${omniworker-agent}/bin/omniworker config set model foo
          check_blocked "config edit" ${omniworker-agent}/bin/omniworker config edit
        ...
        extra-python-packages = let
          ...
          omniworkerWithExtra = omniworker-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "omniworker-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${omniworkerWithExtra}/bin/omniworker || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${omniworkerWithExtra}/bin/omniworker || \
          ...
          if grep -q "PYTHONPATH" ${omniworker-agent}/bin/omniworker; then
        ...
        extra-dependency-groups = let
          omniworkerWithGroups = omniworker-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "omniworker-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${omniworkerWithGroups}"
          echo "venv: ${omniworkerWithGroups.omniworkerVenv}"
        ...
        in pkgs.runCommand "omniworker-config-roundtrip" {
          ...
          merge_and_load() {
            local omniworker_home="$1"
            export OMNIWORKER_HOME="$omniworker_home"
            ${configMergeScript} ${nixSettings} "$omniworker_home/config.yaml"
            ${omniworkerVenv}/bin/python3 -c '
import json, sys

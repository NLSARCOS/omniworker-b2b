# OpenAI-Compatible API Server for Flux Agent Agent
...
SSE streaming. By exposing this endpoint, flux-agent-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  flux-agent-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.flux-agent/.env`
2. Run `flux-agent gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with flux-agent-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns flux-agent-agent as a model) |
...
Authorization: Bearer flux-agent-api-key-here
...
But flux-agent-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "flux-agent-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured flux-agent-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of flux-agent-agent's core prompt
...
Once implemented, flux-agent-agent works as a drop-in backend for:

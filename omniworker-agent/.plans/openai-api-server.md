# OpenAI-Compatible API Server for OmniWorker Agent
...
SSE streaming. By exposing this endpoint, omniworker-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  omniworker-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.omniworker/.env`
2. Run `omniworker gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with omniworker-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns omniworker-agent as a model) |
...
Authorization: Bearer omniworker-api-key-here
...
But omniworker-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "omniworker-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured omniworker-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of omniworker-agent's core prompt
...
Once implemented, omniworker-agent works as a drop-in backend for:

...
# 5. Symlinks the 'omniworker' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ OmniWorker Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/omniworker-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink omniworker into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up omniworker command..."

OMNIWORKER_BIN="$SCRIPT_DIR/venv/bin/omniworker"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$OMNIWORKER_BIN" "$COMMAND_LINK_DIR/omniworker"
echo -e "${GREEN}✓${NC} Symlinked omniworker → $COMMAND_LINK_DISPLAY_DIR/omniworker"
...
# ============================================================================
# Seed bundled skills into ~/.omniworker/skills/
# ============================================================================

OMNIWORKER_SKILLS_DIR="${OMNIWORKER_HOME:-$HOME/.omniworker}/skills"
mkdir -p "$OMNIWORKER_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.omniworker/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$OMNIWORKER_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  omniworker status        # Check configuration"
if is_termux; then
    echo "  omniworker gateway       # Run gateway in foreground"
else
    echo "  omniworker gateway install # Install gateway service (messaging + cron)"
fi
echo "  omniworker cron list     # View scheduled jobs"
echo "  omniworker doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m omniworker_cli.main setup
fi

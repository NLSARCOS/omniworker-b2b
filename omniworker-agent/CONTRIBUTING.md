...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `flux-agent skills browse` (labeled "official") and install it with `flux-agent skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/Flux Agent). Users can install it with `flux-agent skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.flux-agent/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(flux-agent_home, config)` for setup-wizard integration
...
- Integrate with `flux-agent memory setup` via `post_setup()` — no need to touch core code
...
echo "OPENROUTER_API_KEY=***" >> ~/.flux-agent/.env
```
...
├── cli.py                    # Flux AgentCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (flux-agent-cli, flux-agent-telegram, etc.)
├── flux-agent_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── flux-agent_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.flux-agent/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (flux-agent-agent.flux-agent.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.flux-agent/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.flux-agent/`)

| Path | Purpose |
|------|---------|
| `~/.flux-agent/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.flux-agent/.env` | API keys and secrets |
| `~/.flux-agent/auth.json` | OAuth credentials (Nous Portal) |
| `~/.flux-agent/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.flux-agent/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.flux-agent/state.db` | SQLite session database |
| `~/.flux-agent/sessions/` | JSON session logs |
| `~/.flux-agent/cron/` | Scheduled job data |
| `~/.flux-agent/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`flux-agent_state.py`) with full-text search and unique session titles. JSON logs go to `~/.flux-agent/sessions/`.
...
(for example `_FLUX AGENT_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.flux-agent`:
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `flux-agent setup` or update `~/.flux-agent/.env` locally.
...
- **Test it.** Run `flux-agent --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.
...
Create `~/.flux-agent/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `flux-agent_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `flux-agent_cli/skin_engine.py` for the full schema and existing skins as examples.
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the flux-agent wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `flux-agent_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `flux-agent_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `flux-agent_cli/gateway_windows.py::

# Contributing to OmniWorker Agent

Thank you for contributing to OmniWorker Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want OmniWorker to work everywhere.
...
Bundled skills (in `skills/`) ship with every OmniWorker install. They should be **broadly useful to most users**:
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/OmniWorker). Users can install it with `omniworker skills install`.
...
The user may skip setup and keep loading the skill. OmniWorker only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
2. **Tools referenced in SKILL.md prose must be native OmniWorker tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "OmniWorker Agent" is the secondary collaborator. If the contributor's commit shows "OmniWorker Agent" as author because they used OmniWorker to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing OmniWorker tools (`web_extract`, `terminal`, `read_file`).
...
OmniWorker uses a data-driven skin system — no code changes needed to add a new skin.
...
OmniWorker runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
- **Discord**: [discord.gg/OmniWorker](https://discord.gg/OmniWorker) — for questions, showcasing projects, and sharing skills

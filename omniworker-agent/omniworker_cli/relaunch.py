"""
Unified self-relaunch for OmniWorker CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``omniworker sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``omniworker`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched omniworker."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_omniworker_bin() -> Optional[str]:
    """Find the omniworker entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("omniworker")`` on PATH.
    ...
    ``omniworker.exe`` on PATH or the ``sys.executable -m omniworker_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    # PATH lookup
    path_bin = shutil.which("omniworker")
    if path_bin:
        return path_bin
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with omniworker.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_omniworker_bin()
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh omniworker invocation.
    ...
    relaunch contract wants: "run omniworker again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``omniworker.exe`` (a Python console-script shim that wraps
    ...
    That's functionally equivalent — the user sees "omniworker exited, then
    new omniworker started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``omniworker`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nOmniWorker relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run omniworker.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:

"""
Interactive setup wizard for Flux Agent Agent.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Flux Agent Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Flux Agent using environment variables or config commands:")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Flux Agent can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Flux Agent can auto-use for vision,")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Flux Agent auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Flux Agent runs shell commands and code.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Flux Agent delivers cron job results,")
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Flux Agent delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Flux Agent delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Flux Agent to iMessage via BlueBubbles — a free, open-source")
...
# Config values may have different semantics between OpenClaw and Flux Agent.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Flux Agent to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Flux Agent at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Flux Agent at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Flux Agent at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Flux Agent at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Flux Agent equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Flux Agent config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Flux Agent.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Flux Agent's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(omniworker_home: Path) -> bool:
    ...
    print_info("Flux Agent can preview what would be imported before making any changes.")
    print()
    ...
    # Execute the migration — overwrite=False so existing Flux Agent configs are
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Flux Agent (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Flux Agent Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Flux Agent Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Flux Agent Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Flux Agent configured.")
        ...
        setup_mode = prompt_choice("How would you like to set up Flux Agent?", [
...
def _run_quick_setup(config: dict, omniworker_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Flux Agent to messaging apps to chat from anywhere.")

"""
omniworker fallback — manage the fallback provider chain.
...
Subcommands:
  omniworker fallback [list]   Show the current fallback chain (default when no subcommand)
  omniworker fallback add      Pick provider + model via the same picker as `omniworker model`,
                           then append the selection to the chain
  omniworker fallback remove   Pick an entry to delete from the chain
  omniworker fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.omniworker/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from omniworker_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `omniworker model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  omniworker fallback add")
        print()
        return
    ...
    print("  Docs: https://omniworker-agent.omniworker.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `omniworker model`, then append the selection to the chain."""
    from omniworker_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `omniworker model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `omniworker fallback list` to view, or `omniworker fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``omniworker fallback [subcommand]``."""

...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Flux Agent infrastructure directories that only the default profile
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Flux Agent installation itself or a common system binary.  "
...
# ``config.yaml``. ``config.yaml`` is the user-facing Flux Agent config

...
`omniworker tools` and `omniworker setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.omniworker/config.yaml under
...
# who want it opt in via `omniworker tools` → Video Generation, which walks
# them through provider + model selection.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen"}

# Platform-scoped toolsets: only appear in the `omniworker tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``omniworker tools`` → "reconfigure existing" would
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "OMNIWORKER_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "OMNIWORKER_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "OMNIWORKER_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "OMNIWORKER_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "OMNIWORKER_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``omniworker update`` and
      by ``omniworker computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `omniworker update` calls this for every
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_omniworker_home()}/omniworker-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in omniworker-agent directory)")
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.omniworker/config.yaml")
        ...
        # Run the full `omniworker auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.omniworker/.env), then continues straight into PKCE. If they
        ...
        try:
            ...
            _print_info("    Run manually: omniworker auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `omniworker auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: omniworker auth spotify")
        except Exception as exc:
        ...
        try:
            from omniworker_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: omniworker plugins enable observability/langfuse")
        _print_info("    Restart OmniWorker for tracing to take effect.")
        _print_info("    Verify: omniworker plugins list")
    ...
    *,
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"omniworker-{platform}"
    ...
    # "omniworker-cli" (which include all _OMNIWORKER_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[omniworker-cli, spotify]`` after enabling Spotify via ``omniworker
        ...
        # (e.g. "omniworker-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
    ...
    # otherwise saving via `omniworker tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"omniworker-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("omniworker-")}
    ...
    # shipped as a bundled plugin but user must opt in via `omniworker tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `omniworker tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by omniworker tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (omniworker-cli, omniworker-telegram, etc.)
    ...
    # Opening `omniworker tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `omniworker tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `omniworker tools` and `omniworker setup tools`.
    ...
    print(color("  Guide: https://omniworker-agent.omniworker.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color("  Changes take effect on next 'omniworker' or gateway restart.", Colors.DIM))
    print()

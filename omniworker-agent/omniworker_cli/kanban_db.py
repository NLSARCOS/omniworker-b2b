...
the profile's OmniWorker home but are otherwise isolated: a worker spawned
...
def kanban_home() -> Path:
    """Return the shared OmniWorker root that anchors the kanban board.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a OmniWorker
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real OmniWorker profile.
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default OmniWorker root exists

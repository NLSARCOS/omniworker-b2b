...
Implements the same 3-step registration flow as dingtalk-omniworker-connector:
...
def _ensure_qrcode_installed() -> bool:
    ...
    # Try uv first (OmniWorker convention), then pip
    for cmd in (
...
def dingtalk_qr_auth() -> Optional[Tuple[str, str]]:
    ...
    """
    from omniworker_cli.setup import print_info, print_success, print_warning, print_error

    print()
    print_info("  Initializing DingTalk device authorization...")
    print_info("  Note: the scan page is branded 'OmniWorker' — DingTalk's")

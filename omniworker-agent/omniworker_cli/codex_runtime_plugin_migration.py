"""Migrate Flux Agent' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Flux Agent' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Flux Agent mcp_servers.<n>.command/args/env  → codex stdio transport
  Flux Agent mcp_servers.<n>.url/headers       → codex streamable_http transport
  Flux Agent mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Flux Agent mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Flux Agent-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Flux Agent' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Flux Agent config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Flux Agent' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one Flux Agent MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # Flux Agent' transport: sse hint is informational; codex auto-negotiates
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Flux Agent)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Flux Agent' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Flux Agent' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Flux Agent
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Flux Agent' own
...
def migrate(
    ...
    *,
    ...
    """Translate Flux Agent mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.
    ...
    Args:
        ...
        expose_hermes_tools: when True (default), register Flux Agent' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Flux Agent for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Flux Agent config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject Flux Agent' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Flux Agent for the tools codex

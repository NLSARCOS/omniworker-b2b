"""
Doctor command for omniworker CLI.
...
_DHH = display_omniworker_home()  # user-facing display path (e.g. ~/.omniworker or ~/.omniworker/profiles/coder)

# Load environment variables from ~/.omniworker/.env so API key checks work
_env_path = get_env_path()
load_omniworker_dotenv(omniworker_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.omniworker/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `omniworker`.
    os.environ.setdefault("OMNIWORKER_INTERACTIVE", "1")

    # Handle `omniworker doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.omniworker/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`omniworker doctor --ack {hit.advisory.id}`."
                )
    ...
    # Check ~/.omniworker/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'omniworker setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'omniworker setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'omniworker setup' to create one")
                issues.append("Run 'omniworker setup' to create .env")
    
    # Check ~/.omniworker/config.yaml (primary) or project cli-config.yaml (fallback)
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'omniworker config set model.provider <valid_provider>'"
                    )
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.omniworker/.env or run 'omniworker setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'omniworker setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'omniworker config set model.provider <name>'"
                            )
                except Exception:
                    pass
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'omniworker setup' to migrate config")
                else:
                    issues.append("Run 'omniworker doctor --fix' or 'omniworker setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'omniworker doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # `omniworker auth openai-codex` aren't told they're missing something.
        check_info(
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'omniworker doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (omniworker bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "omniworker"
        ...
        _cmd_link = _cmd_link_dir / "omniworker"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(omniworker not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/omniworker → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/omniworker points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/omniworker → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/omniworker — run 'omniworker doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/omniworker exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/omniworker not found",
                    "(omniworker command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/omniworker → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/omniworker symlink — run 'omniworker doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'omniworker-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'omniworker-agent[vercel]'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `omniworker doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'omniworker config set model.provider <provider>' "
    ...
    # never the legitimate source for `omniworker doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'omniworker setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: omniworker skills list)")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: omniworker memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: omniworker memory setup")
                issues.append("No Honcho API key — run 'omniworker memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run omniworker memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: omniworker memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: omniworker memory setup")
        except Exception as _e:
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "omniworker -p" in content:
                            _m = _re.search(r"omniworker -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'omniworker doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))

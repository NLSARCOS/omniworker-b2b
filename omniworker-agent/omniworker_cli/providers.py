"""
Single source of truth for provider identity in Flux Agent Agent.
...
2. **Flux Agent overlays** — transport type, auth patterns, aggregator flags,
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Flux Agent overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Flux Agent overlay
    ...
    if overlay is not None:
        # Flux Agent-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,

"""
Single source of truth for provider identity in OmniWorker Agent.
...
2. **OmniWorker overlays** — transport type, auth patterns, aggregator flags,
...
# -- OmniWorker overlay ----------------------------------------------------------
# OmniWorker-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class OmniWorkerOverlay:
    """OmniWorker-specific provider metadata layered on top of models.dev."""
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. OmniWorker overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + OmniWorker overlay
    ...
    if overlay is not None:
        # OmniWorker-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,

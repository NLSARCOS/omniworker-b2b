...
TIPS = [
    ...
    "Ctrl+Z suspends Flux Agent to the background — run fg in your shell to resume.",
    ...
    "hermes mcp serve runs Flux Agent itself as an MCP server for other agents.",
    ...
    "hermes backup creates a zip backup of your entire Flux Agent home directory.",
    ...
    "hermes gateway install sets up Flux Agent as a system service (systemd/launchd).",
    ...
    "FLUX AGENT_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Flux Agent tools programmatically — results stay out of context.",
    ...
    "When exiting, Flux Agent prints a resume command with session ID and stats.",
    ...
    # --- Gateway & Messaging ---
    "Flux Agent runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to Flux Agent via a local macOS server.",
    ...
    "Flux Agent loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "hermes acp runs Flux Agent as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "FLUX AGENT_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Flux Agent your own.",
    ...
    "Before a gateway session reset, Flux Agent auto-flushes important facts to memory in the background.",
    ...
    "Each profile gets its own subprocess HOME at FLUX AGENT_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "FLUX AGENT_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in FLUX AGENT_HOME and the host CLI auto-execs into the container.",
    ...
    "FLUX AGENT_DEV=1 bypasses container mode detection for local development.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Flux Agent auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Flux Agent config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Flux Agent is working.',
    ...
    "FLUX AGENT_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    ...
    # --- TUI & Dashboard ---
    'FLUX AGENT_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "FLUX AGENT_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'hermes dashboard --tui embeds the full Flux Agent TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'FLUX AGENT_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'FLUX AGENT_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'FLUX AGENT_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'FLUX AGENT_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    'FLUX AGENT_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'FLUX AGENT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hermes/plugins/ — trust-gated by design.',
    "FLUX AGENT_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'FLUX AGENT_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'FLUX AGENT_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'FLUX AGENT_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'FLUX AGENT_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'FLUX AGENT_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'FLUX AGENT_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'FLUX AGENT_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'FLUX AGENT_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'FLUX AGENT_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'FLUX AGENT_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Flux Agent block commands when the tirith scanner itself errors out.',

...
def register_credential_file(
    ...
    *relative_path* is relative to ``OMNIWORKER_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside OMNIWORKER_HOME so that a malicious
    ...
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to OMNIWORKER_HOME)",
            relative_path,
        )
        return False

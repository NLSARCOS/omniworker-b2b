...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("OMNIWORKER_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("OMNIWORKER_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set OMNIWORKER_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind OMNIWORKER_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds OMNIWORKER_SESSION_PLATFORM via
    ...
    """
    if os.getenv("OMNIWORKER_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $OMNIWORKER_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_OMNIWORKER_ENV_PATH = (
    r'(?:~\/\.omniworker/|'
    r'(?:\$home|\$\{home\})/\.omniworker/|'
    r'(?:\$omniworker_home|\$\{omniworker_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bomniworker\s+gateway\s+(stop|restart)\b', "stop/restart omniworker gateway (kills running agents)"),
    (r'\bomniworker\s+update\b', "omniworker update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart omniworker-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart omniworker-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(omniworker|gateway|cli\.py)\b', "kill omniworker/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill omniworker` but not
    # `kill -9 $(pgrep -f omniworker)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("OMNIWORKER_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("OMNIWORKER_INTERACTIVE")
    ...
    if is_gateway or os.getenv("OMNIWORKER_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("OMNIWORKER_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("OMNIWORKER_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = os.getenv("OMNIWORKER_EXEC_ASK")

...
`mcp_tool.py`, and `flux-agent_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# Flux AgentMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_flux-agent_provider_class() -> Optional[type]:
    ...
    class Flux AgentMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._flux-agent_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`Flux AgentTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, Flux AgentTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._flux-agent_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._flux-agent_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import Flux AgentTokenStorage
                        if isinstance(storage, Flux AgentTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._flux-agent_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import Flux AgentTokenStorage
            if not isinstance(storage, Flux AgentTokenStorage):
                return
    ...
    return Flux AgentMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_FLUX AGENT_PROVIDER_CLS: Optional[type] = _make_flux-agent_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`Flux AgentMCPOAuthProvider` directly using the helpers
        ...
        return _FLUX AGENT_PROVIDER_CLS(
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``flux-agent mcp remove <name>`` and (indirectly) by
        ``flux-agent mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:

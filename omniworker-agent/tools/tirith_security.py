...
it is automatically downloaded from GitHub releases to $FLUX AGENT_HOME/bin/tirith.
...
def _get_flux-agent_home() -> str:
    """Return the Flux Agent home directory, respecting FLUX AGENT_HOME env var."""
    return str(get_flux-agent_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_flux-agent_home(), ".tirith-install-failed")
...
def _flux-agent_bin_dir() -> str:
    """Return $FLUX AGENT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_flux-agent_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $FLUX AGENT_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_flux-agent_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $FLUX AGENT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $FLUX AGENT_HOME/bin/tirith
    ...
    flux-agent_bin = os.path.join(_flux-agent_bin_dir(), "tirith")
    if os.path.isfile(flux-agent_bin) and os.access(flux-agent_bin, os.X_OK):
        _resolved_path = flux-agent_bin
        _install_failure_reason = ""
        _clear_install_failed()

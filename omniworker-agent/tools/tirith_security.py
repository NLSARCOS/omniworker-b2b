...
it is automatically downloaded from GitHub releases to $OMNIWORKER_HOME/bin/tirith.
...
def _get_omniworker_home() -> str:
    """Return the OmniWorker home directory, respecting OMNIWORKER_HOME env var."""
    return str(get_omniworker_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_omniworker_home(), ".tirith-install-failed")
...
def _omniworker_bin_dir() -> str:
    """Return $OMNIWORKER_HOME/bin, creating it if needed."""
    d = os.path.join(_get_omniworker_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $OMNIWORKER_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_omniworker_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $OMNIWORKER_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $OMNIWORKER_HOME/bin/tirith
    ...
    omniworker_bin = os.path.join(_omniworker_bin_dir(), "tirith")
    if os.path.isfile(omniworker_bin) and os.access(omniworker_bin, os.X_OK):
        _resolved_path = omniworker_bin
        _install_failure_reason = ""
        _clear_install_failed()

...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Flux Agent exit
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    # interactive Flux Agent password prompt or the sudo -S password-pipe path.
    ...
    # silently without Flux Agent prompting.
...
Do NOT use shell background wrappers (nohup/disown/setsid/trailing '&'); use background=true so Flux Agent tracks lifecycle and output.
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Flux Agent can track the process, then run "

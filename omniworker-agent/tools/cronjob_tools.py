...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via omniworker model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    # Only relative paths within ~/.omniworker/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.omniworker/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.omniworker/scripts/ and use just the filename."
        )
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("OMNIWORKER_INTERACTIVE")
        or os.getenv("OMNIWORKER_GATEWAY_SESSION")
        or os.getenv("OMNIWORKER_EXEC_ASK")
    )

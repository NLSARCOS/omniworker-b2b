"""
Lazy dependency installer for opt-in Flux Agent Agent backends.

Many Flux Agent features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``omniworker-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``omniworker tools`` or the manual pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # sqlite-vec: vector search for NativeMemory semantic recall
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("OMNIWORKER_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``omniworker update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``omniworker_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``omniworker update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``omniworker update``. Never raises; lazy-install failures

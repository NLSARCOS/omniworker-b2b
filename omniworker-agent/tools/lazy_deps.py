...
(``flux-agent-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``flux-agent tools`` or the manual pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # Dashboard (`flux-agent dashboard`)
    "tool.dashboard": (
        "fastapi==0.133.1",
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("FLUX AGENT_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``flux-agent update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``flux-agent_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``flux-agent update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``flux-agent update``. Never raises; lazy-install failures

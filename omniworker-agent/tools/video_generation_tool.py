...
provider** — turn on a backend by enabling a plugin (``flux-agent plugins
enable video_gen/<name>``) and selecting it in ``flux-agent tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/flux-agent-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `flux-agent tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `flux-agent plugins list` to see "
            f"installed video gen backends, or `flux-agent tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `flux-agent tools` → "
...
# `flux-agent tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`flux-agent tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `flux-agent tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}

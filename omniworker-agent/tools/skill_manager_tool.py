...
~/.flux-agent/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `flux-agent config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.flux-agent/skills/ (single source of truth)
OMNIWORKER_HOME = get_flux-agent_home()
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`flux-agent curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.flux-agent/skills/) first, then any
...
SKILL_MANAGE_SCHEMA = {
    "name": "skill_manage",
    "description": (
        "Manage skills (create, update, delete) — your procedural memory of "
        "reusable approaches for recurring tasks. "
        f"New skills go to {display_flux-agent_home()}/skills/; existing skills are modified wherever they live.\n\n"
        "Actions: create (full SKILL.md + optional category), "
        "patch (old_string/new_string — preferred for fixes), "
        "edit (full SKILL.md rewrite — major overhauls only), "
        "delete, write_file, remove_file.\n\n"
        "On delete, pass `absorbed_into=<umbrella>` when merging this skill into "
        "another (the target must already exist — create/patch it first), or "
        "`absorbed_into=\"\"` when pruning with no target. This lets the curator "
        "distinguish consolidation from pruning so downstream consumers (e.g. cron "
        "jobs referencing the old name) update correctly.\n\n"
        "Create when: a complex task succeeded (5+ calls), errors were overcome, a "
        "user-corrected approach worked, a non-trivial workflow was discovered, or "
        "the user asks to remember a procedure.\n"
        "Update when: instructions are stale/wrong or missing steps/pitfalls found "
        "during use — patch immediately if a skill you used had gaps.\n"
        "After difficult tasks, offer to save a skill; skip simple one-offs. Confirm "
        "with the user before creating/deleting.\n\n"
        "Good skills have: trigger conditions, numbered steps with exact commands, a "
        "pitfalls section, verification steps. Use skill_view() for format examples.\n\n"
        "Pinned skills are protected from deletion only — delete refuses, pointing "
        "the user to `flux-agent curator unpin <name>`; patch/edit still work so you "
        "can keep improving them."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "absorbed_into": {
                "type": "string",
                "description": (
                    "For 'delete' only — declares intent. Pass the umbrella skill "
                    "name when this skill was merged into another (target must "
                    "already exist), or an empty string when pruning a stale skill "
                    "with no target. Omitting it works for backward compatibility "
                    "but downstream tooling (e.g. cron-job reference rewriting) "
                    "then has to guess at intent."
                )
            },
        },

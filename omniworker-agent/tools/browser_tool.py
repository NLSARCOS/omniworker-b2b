...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    hermes_node_bin = str(flux-agent_home / "node" / "bin")
    hermes_node_root = str(flux-agent_home / "node")
    hermes_nm_bin = str(flux-agent_home / "node_modules" / ".bin")
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Flux Agent silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Flux Agent CLI where sys.stdout and sys.stderr both report
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Flux Agent-managed
    ...
    # Build an extended search PATH including Flux Agent-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                hermes_nm = str(get_flux-agent_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=hermes_nm)
            if not recheck:
                hermes_node_bin = str(get_flux-agent_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=hermes_node_bin)
            if not recheck:
                hermes_node_root = str(get_flux-agent_home() / "node")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = flux-agent_home / "browser_recordings"

...
def iter_sync_files(container_base: str = "/root/.flux-agent") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.flux-agent to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.flux-agent", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync_back(self, flux-agent_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.flux-agent/`` directory as a tar archive,
        ...
        # against an uninitialized remote .flux-agent/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (flux-agent_home or get_flux-agent_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="flux-agent-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.flux-agent/skills/a.md`` →
        ``~/.flux-agent/skills/a.md``, a new remote file at
        ``/root/.flux-agent/skills/b.md`` maps to ``~/.flux-agent/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:

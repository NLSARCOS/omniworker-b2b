...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/flux-agent-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/flux-agent-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/flux-agent-results"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/flux-agent-results"
    return STORAGE_DIR

...
CHECKPOINT_BASE = get_flux-agent_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("FLUX AGENT_CHECKPOINT_TIMEOUT", "30"))))
...
# look for ``HEAD``/``info/exclude``/``FLUX AGENT_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at FLUX AGENT_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "FLUX AGENT_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "FLUX AGENT_WORKDIR"
            if wd_marker.exists():
                try:

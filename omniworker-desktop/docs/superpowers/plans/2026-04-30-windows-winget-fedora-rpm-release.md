...
linux:
  ...
  description: |
    ...
    with OmniWorker Agent — a self-improving AI assistant with tool use, multi-platform
...
PackageIdentifier: OmniWorker.OmniWorkerDesktop
...
describe("generateWingetManifests", () => {
  it("produces three YAML files under the winget-pkgs directory layout", () => {
    ...
    expect(existsSync(join(outDir, "OmniWorker.OmniWorkerDesktop.installer.yaml"))).toBe(true);
    expect(existsSync(join(outDir, "OmniWorker.OmniWorkerDesktop.locale.en-US.yaml"))).toBe(true);
    expect(existsSync(join(outDir, "OmniWorker.OmniWorkerDesktop.yaml"))).toBe(true);
  });
  ...
  it("replaces all placeholders in the installer manifest", () => {
    ...
    const outFile = join(
      ...
      "OmniWorkerDesktop",
      "9.9.9",
      "OmniWorker.OmniWorkerDesktop.installer.yaml",
    );
  ...
  it("replaces ReleaseNotesUrl in the locale manifest", () => {
    ...
    const outFile = join(
      ...
      "OmniWorkerDesktop",
      "9.9.9",
      "OmniWorker.OmniWorkerDesktop.locale.en-US.yaml",
    );
    const content = readFileSync(outFile, "utf-8");
    expect(content).toContain(
...
// the result under dist/winget/manifests/n/OmniWorker/OmniWorkerDesktop/<version>/.
//
...
export function generateWingetManifests({ rootDir, version, name, publishOwner }) {
  ...
  const installerUrl =
    `https://github.com/${publishOwner}/omniworker-desktop/releases/download/v${version}/${name}-${version}-setup.exe`;
  const releaseNotesUrl =
    `https://github.com/${publishOwner}/omniworker-desktop/releases/tag/v${version}`;
  ...
  const files = [
    ["Installer.template.yaml", "OmniWorker.OmniWorkerDesktop.installer.yaml"],
    ["Locale.en-US.template.yaml", "OmniWorker.OmniWorkerDesktop.locale.en-US.yaml"],
    ["Version.template.yaml", "OmniWorker.OmniWorkerDesktop.yaml"],
  ];
    ...
    steps:
      ...
      - name: Publish GitHub release
        ...
        with:
          ...
          name: OmniWorker Desktop ${{ needs.prepare.outputs.tag }}
...
Run: `gh run download <run-id> -n winget-manifests-0.2.3 -D /tmp/winget-check && ls -la /tmp/winget-check/manifests/n/OmniWorker/OmniWorkerDesktop/0.2.3/`
...
Run: `cat /tmp/winget-check/manifests/n/OmniWorker/OmniWorkerDesktop/0.2.3/*.yaml`

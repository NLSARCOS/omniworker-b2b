export default {
  ...
  empty: "(空)",
  dataSection: "数据",
  dataHint: "导出或导入你的 OmniWorker 配置、会话、技能和记忆。",
  backingUp: "正在备份...",
  exportBackup: "导出备份",
  importing: "正在导入...",
  importBackup: "导入备份",
  ...
  modeLocal: "本地",
  modeRemote: "远程",
  modeLocalHint: "使用本机安装的 OmniWorker",
  modeRemoteHint: "连接到网络或云服务器上的 OmniWorker API",
  remoteUrl: "远程服务器地址",
  remoteUrlHint:
  ...
  serverConfigHint:
    "你已连接到远程 OmniWorker 服务器。模型选择、提供商 API Key 和凭据均在服务器的 <code>~/.omniworker/.env</code> 和 <code>config.yaml</code> 中管理。请在主机上编辑（例如 <code>docker exec -it omniworker vi /opt/data/.env</code>）然后重启容器。",
  connectionMode: "模式",

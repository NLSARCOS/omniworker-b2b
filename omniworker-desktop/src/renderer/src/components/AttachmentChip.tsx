...
export function AttachmentChip({
  ...
  const showImageMenu = (event: React.MouseEvent): void => {
    ...
    window.omniworkerAPI.showMediaMenu(attachment.dataUrl, attachment.name, {
      open: t("chat.media.open") || "Open",
      saveAs: t("chat.media.saveAs") || "Save as...",
    });
  };
        ...
        >
          ...
          >
            <button
              ...
              onClick={() =>
                window.omniworkerAPI.saveMediaFile(
                  attachment.dataUrl!,
                  attachment.name,
                )
              }
            >
              <Download size={14} />
              {t("chat.media.saveImage") || "Save image"}
            </button>
            <button
              className="chat-image-preview-btn"
              onClick={() => setZoomed(false)}

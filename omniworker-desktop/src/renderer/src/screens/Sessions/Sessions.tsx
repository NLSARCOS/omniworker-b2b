...
function Sessions({
  ...
  const confirmDelete = useCallback(
    async (sessionId: string): Promise<void> => {
      ...
      try {
        await window.omniworkerAPI.deleteSession(sessionId);
      } catch (err) {
  ...
  const confirmBulkDelete = useCallback(
    async (sessionIds: string[]): Promise<void> => {
      ...
      try {
        await window.omniworkerAPI.deleteSessions(ids);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      try {
        const results = await window.omniworkerAPI.searchSessions(query);

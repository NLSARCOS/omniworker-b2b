...
export default function OneChatModal({
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      try {
        const items = await window.omniworkerAPI.getSessionMessages(sessionId);
        if (cancelled) return;
        const loaded: ChatMessage[] = items
          .filter((it) => it.role === "user" || it.role === "assistant")
          .map((it) => ({
            id: `db-${it.id}`,
            role: it.role === "user" ? "user" : "agent",
            text: it.content || "",
  ...
  const handleSend = async (): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        const items = await window.omniworkerAPI.getSessionMessages(reloadSessionId);
        const loaded: ChatMessage[] = items
          .filter((it) => it.role === "user" || it.role === "assistant")
          .map((it) => ({
            id: `db-${it.id}`,

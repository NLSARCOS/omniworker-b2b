 ...
 * bytes below target without falling below the quality / resolution caps.
 */
export async function compressImageToFit(
  ...
  if (file.type === "image/gif") {
    // Canvas only draws the first frame of an animated GIF; we can't
    // compress the animation itself, so refuse to transcode it.
  ...
  // Check if WebP is actually supported by the canvas. Opaque targets
  // compress ~5-10× better in WebP than PNG, so we prefer it, but fall
  // back to JPEG if WebP isn't honoured.
  let targetMime = outputMime;
  let targetExt = ext;
  if (!hasAlpha) {
    const hasWebp = await canvasSupportsType(canvas, "image/webp");
    if (!hasWebp) {
  ...
  const newName = `${cleanName}_compressed${targetExt}`;

  // Compression loop: drop quality, then scale down
  let scale = 1.0;
  let quality = 0.85;
  let workingCanvas = canvas;

  // We run up to 12 iterations (typical convergence is 1-3 ticks).
  for (let tick = 0; tick < 12; tick++) {
    let bestBlob: Blob | null = null;
    if (targetMime === "image/png") {
      // PNG uses lossless DEFLATE compression, quality parameter is ignored.
      bestBlob = await canvasToBlob(workingCanvas, "image/png");
    } else {
      // For lossy targets (WebP / JPEG), compress at current quality.
      // Additionally, also test JPEG at the same quality if using WebP:
      // at low resolutions WebP sometimes bloats above JPEG.
      const candidate1 = await canvasToBlob(workingCanvas, targetMime, quality);
      let bestCandidate = candidate1;

      if (targetMime === "image/webp") {
        try {
          const candidate2 = await canvasToBlob(workingCanvas, "image/jpeg", quality);
          if (candidate2.size < candidate1.size) {
            bestCandidate = candidate2;
          }
        } catch {
          // Fall back to WebP candidate
        }
      }
      bestBlob = bestCandidate;
    }

    if (bestBlob && bestBlob.size <= targetBytes) {
      return new File([bestBlob], newName, { type: bestBlob.type });
    }

    // For lossy formats: drop quality before scaling — visually preferable.
    if (targetMime !== "image/png" && quality > 0.5) {
      quality -= 0.15;
      continue;
    }
...
export async function processFiles(
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    let path = "";
    try {
      path = window.omniworkerAPI.getPathForFile(file) || "";
    } catch {
      path = "";
    }
    ...
    if (!path) {
      ...
      try {
        ...
        path = await window.omniworkerAPI.stageAttachment(
          options.sessionId || "",
          name,
          base64,
        );

  ...
  function ChatInput(
    ...
    const formatError = useCallback(
      (err: AttachmentError): string => {
        switch (err.code) {
          case "too-many":
            return t("chat.attachTooMany") || "Too many attachments";
          case "image-too-large":
            return t("chat.attachImageTooLarge", { name: err.filename }) || `${err.filename} is too large`;
          case "text-too-large":
            return t("chat.attachTextTooLarge", { name: err.filename }) || `${err.filename} is too large`;
          case "unsupported-type":
            return t("chat.attachUnsupported", { name: err.filename }) || `Unsupported type for ${err.filename}`;
          case "read-failed":
            return t("chat.attachReadFailed", { name: err.filename }) || `Failed to read ${err.filename}`;
          default:
            return err.filename;
        }
      },
      ...
      <>
        ...
        <div className={`chat-input-wrapper ${isPlanExpired ? "opacity-60 cursor-not-allowed bg-zinc-900" : ""}`}>
          ...
          <button
            ...
            disabled={isLoading || isPlanExpired}
          ...
          {isLoading ? (
            ...
            <>
              {hasSession && !isPlanExpired && (input.trim() || attachments.length > 0) && (
                <button
              ...
              <button
                ...
                onClick={handleSend}
                disabled={!canSend || isPlanExpired}
                title={t("chat.send")}
              >
                <Send size={16} />
              </button>

...
function Schedules({ profile }: SchedulesProps): React.JSX.Element {
  ...
  const loadJobs = useCallback(async (): Promise<void> => {
    try {
      const list = await window.flux-agentAPI.listCronJobs(true, profile);
      setJobs(list);
    } catch {
  ...
  async function handleCreate(): Promise<void> {
    ...
    setError("");
    try {
      const result = await window.flux-agentAPI.createCronJob(
        buildSchedule(),
  ...
  async function handleRemove(jobId: string): Promise<void> {
    ...
    setError("");
    try {
      const result = await window.flux-agentAPI.removeCronJob(jobId, profile);
  ...
  async function handleToggle(job: CronJob): Promise<void> {
    ...
    try {
      const result =
        job.state === "paused"
          ? await window.flux-agentAPI.resumeCronJob(job.id, profile)
          : await window.flux-agentAPI.pauseCronJob(job.id, profile);
      if (result.success) {
        await loadJobs();
      } else {
  ...
  async function handleTrigger(jobId: string): Promise<void> {
    ...
    setError("");
    try {
      const result = await window.flux-agentAPI.triggerCronJob(jobId, profile);
      if (result.success) {
        await loadJobs();
      } else {

...
function Schedules({ profile }: SchedulesProps): React.JSX.Element {
  ...
  const loadJobs = useCallback(async (): Promise<void> => {
    try {
      const list = await window.omniworkerAPI.listCronJobs(true, profile);
      setJobs(list);
    } catch {
  ...
  async function handleCreate(): Promise<void> {
    ...
    setError("");
    try {
      const result = await window.omniworkerAPI.createCronJob(
        buildSchedule(),
  ...
  async function handleRemove(jobId: string): Promise<void> {
    ...
    setError("");
    try {
      const result = await window.omniworkerAPI.removeCronJob(jobId, profile);
  ...
  async function handleToggle(job: CronJob): Promise<void> {
    ...
    try {
      const result =
        job.state === "paused"
          ? await window.omniworkerAPI.resumeCronJob(job.id, profile)
          : await window.omniworkerAPI.pauseCronJob(job.id, profile);
      if (result.success) {
        await loadJobs();
      } else {
  ...
  async function handleTrigger(jobId: string): Promise<void> {
    ...
    setError("");
    try {
      const result = await window.omniworkerAPI.triggerCronJob(jobId, profile);
      if (result.success) {
        await loadJobs();
      } else {

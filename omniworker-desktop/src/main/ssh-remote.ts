...
export async function sshInstallSkill(config: SshConfig, identifier: string): Promise<{ success: boolean; error?: string }> {
  try {
    await sshExec(config, `omniworker skills install ${shellQuote(identifier)} --yes 2>&1`, undefined, 120000);
    return { success: true };
  } catch (err) {
...
export async function sshUninstallSkill(config: SshConfig, name: string): Promise<{ success: boolean; error?: string }> {
  try {
    await sshExec(config, `omniworker skills uninstall ${shellQuote(name)} 2>&1`);
    return { success: true };
  } catch (err) {
    return { success: false, error: (err as Error).message };
  }
...
const DEFAULT_SOUL = `You are OmniWorker, a helpful AI assistant. You are friendly, knowledgeable, and always eager to help.
...
function remoteSoulPath(profile?: string): string {
  if (profile && profile !== "default") return `~/.omniworker/profiles/${profile}/SOUL.md`;
...
function remoteConfigPath(profile?: string): string {
  if (profile && profile !== "default") return `$HOME/.omniworker/profiles/${profile}/config.yaml`;
...
function remoteEnvPath(profile?: string): string {
  if (profile && profile !== "default") return `~/.omniworker/profiles/${profile}/.env`;
...
export function sshGetOmniWorkerHome(_config: SshConfig, profile?: string): string {
  if (profile && profile !== "default") return `~/.omniworker/profiles/${profile}`;
  return "~/.omniworker";
}
...
`;
  try {
    ...
    return [{ name: "default", path: "~/.omniworker", isDefault: true, isActive: true, model: "", provider: "auto", hasEnv: false, hasSoul: false, skillCount: 0, gatewayRunning: false }];
  }
}
...
export async function sshCreateProfile(
  ...
  try {
    ...
    if (clone) {
      await sshExec(config, `omniworker profiles create ${quoted} --clone-from default 2>&1 || mkdir -p ~/.omniworker/profiles/${quoted}`);
    } else {
      await sshExec(config, `omniworker profiles create ${quoted} 2>&1 || mkdir -p ~/.omniworker/profiles/${quoted}`);
    }
    return true;
  } catch {
    return false;
...
export async function sshDeleteProfile(config: SshConfig, name: string): Promise<boolean> {
  try {
    ...
    const quoted = shellQuote(safe);
    await sshExec(config, `omniworker profiles delete ${quoted} --yes 2>&1 || rm -rf ~/.omniworker/profiles/${quoted}`);
    return true;
  } catch {
    return false;
  }
...
export async function sshStartGateway(config: SshConfig): Promise<void> {
  try {
    await sshExec(config, `nohup omniworker gateway start > $HOME/.omniworker/gateway.log 2>&1 &`);
  } catch {
    // best effort
  }
}
...
export async function sshGetOmniWorkerVersion(config: SshConfig): Promise<string | null> {
  try {
    const out = await sshExec(config, `omniworker --version 2>/dev/null || omniworker version 2>/dev/null || echo ""`);
...
export async function sshGetPlatformEnabled(
  ...
  void profile;
  try {
    const raw = await sshReadFile(config, "$HOME/.omniworker/gateway_state.json");
    if (raw.trim()) {
...
export async function sshRunDoctor(config: SshConfig): Promise<string> {
  try {
    const out = await sshExec(config, `omniworker doctor 2>&1 || echo "omniworker not found in PATH"`);
...
export async function sshSaveModels(config: SshConfig, models: SavedModel[]): Promise<void> {
  await sshWriteFile(config, "$HOME/.omniworker/models.json", JSON.stringify(models, null, 2));
}

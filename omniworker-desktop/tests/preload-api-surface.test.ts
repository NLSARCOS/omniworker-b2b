...
/**
 * Extract method names from the OmniWorkerAPI interface in index.d.ts.
 */
function extractTypeMethods(src: string): string[] {
  const methods: string[] = [];
  // Match lines inside `interface OmniWorkerAPI { ... }`
  const interfaceMatch = src.match(
    /interface\s+OmniWorkerAPI\s*\{([\s\S]*?)^\}/m,
  );
...
describe("New APIs from v0.8/v0.9 features", () => {
  it("has backup/import APIs", () => {
    expect(preloadMethods).toContain("runOmniWorkerBackup");
    expect(preloadMethods).toContain("runOmniWorkerImport");
    expect(typeMethods).toContain("runOmniWorkerBackup");
    expect(typeMethods).toContain("runOmniWorkerImport");
  });
  ...
  it("has debug dump API", () => {
    expect(preloadMethods).toContain("runOmniWorkerDump");
    expect(typeMethods).toContain("runOmniWorkerDump");
  });

...
// ─── OmniWorker auth credential discovery ─────────────────

describe("OmniWorker auth credential discovery", () => {
  async function importInstallerWithHome(
    home: string,
  ): Promise<typeof import("../src/main/installer")> {
    vi.resetModules();
    process.env.OMNIWORKER_HOME = home;
    return await import("../src/main/installer");
  }

  afterEach(() => {
    delete process.env.OMNIWORKER_HOME;
    vi.resetModules();
  });
  ...
  it("detects OAuth credentials stored in auth.json credential_pool", async () => {
    ...
    const { OMNIWORKER_AUTH_FILE, hasOmniWorkerAuthCredential } =
      await importInstallerWithHome(TEST_DIR);

    expect(OMNIWORKER_AUTH_FILE).toBe(join(TEST_DIR, "auth.json"));
    expect(hasOmniWorkerAuthCredential("openai-codex")).toBe(true);
    expect(hasOmniWorkerAuthCredential("anthropic")).toBe(false);
  });
  ...
  it("accepts active_provider and providers entries as configured credentials", async () => {
    ...
    const { hasOmniWorkerAuthCredential } = await importInstallerWithHome(TEST_DIR);

    expect(hasOmniWorkerAuthCredential("openrouter")).toBe(true);
    expect(hasOmniWorkerAuthCredential("anthropic")).toBe(true);
    expect(hasOmniWorkerAuthCredential("openai-codex")).toBe(false);
  });

  it("returns false when auth.json is missing or malformed", async () => {
    const missing = await importInstallerWithHome(TEST_DIR);
    expect(missing.hasOmniWorkerAuthCredential("openai-codex")).toBe(false);

    writeFileSync(join(TEST_DIR, "auth.json"), "{not-json");
    const malformed = await importInstallerWithHome(TEST_DIR);
    expect(malformed.hasOmniWorkerAuthCredential("openai-codex")).toBe(false);
  });
});

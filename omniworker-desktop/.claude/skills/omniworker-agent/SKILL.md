...
# OmniWorker Agent - Complete Project Guide (A-Z)

> **Purpose of this document:** A single, comprehensive reference that explains everything about the OmniWorker Agent project — its architecture, source code, features, release history, and design patterns — so that any AI or developer can fully understand the system.
...
- **MCP integration** — both client (connect to any MCP server) and server (expose OmniWorker to MCP clients)
...
│   ├── server.py             # OmniWorkerACPAgent class
...
The `OmniWorkerCLI` class provides the interactive terminal interface.
...
- `_apply_profile_override()` runs BEFORE any imports to set `OMNIWORKER_HOME`
...
OmniWorker has a pluggable memory provider interface:
...
- `X-OmniWorker-Session-Id` header for persistent sessions
...
**Base Class:** `OmniWorkerAgentBaseEnv` (extends `atroposlib.BaseEnv`)

**Configuration:**

```python
OmniWorkerAgentEnvConfig:
...
- `OmniWorkerAgentLoop` — Orchestrates step-by-step rollouts
...
Run multiple fully isolated OmniWorker instances from the same installation.

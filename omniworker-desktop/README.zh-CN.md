...
- Flux Agent Agent 的首次引导式安装
- OpenRouter、Anthropic、OpenAI 以及本地 OpenAI 兼容端点的提供商配置
- 基于 Flux Agent CLI 的流式聊天界面
- 带恢复和搜索能力的会话历史
- 用于隔离 Flux Agent 环境的档案切换
- 对人格、记忆、工具和已安装技能的图形界面访问
- Flux Agent 消息集成的网关控制
...
聊天请求会通过本地 Flux Agent CLI 发出，桌面应用再把响应流式回传到 UI 中。
...
- Node.js 和 npm
- 可运行 Flux Agent 安装器的类 Unix shell 环境
- 首次安装时用于下载 Flux Agent 的网络访问能力
...
应用首次打开时，会自动检测是否存在现有 Flux Agent 安装；如果没有，会引导你完成安装。
...
- 桌面应用依赖上游 Flux Agent Agent 项目来完成代理行为和工具执行。
- 内置安装器会以 `--skip-setup` 运行官方 Flux Agent 安装脚本，再在 GUI 中完成提供商配置。
